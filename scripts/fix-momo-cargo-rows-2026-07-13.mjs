...
if (dup.length) { const { rows } = await c.query(`select * from tb_forwarder where id = any($1::text[])`, [dup.map(r=>r.id)]); backup.phase1_delete = rows; }
if (fixable.length) { const { rows } = await c.query(`select * from tb_forwarder where id = any($1::text[])`, [fixable.map(b=>b.id)]); backup.phase2_before = rows; }
const bf = `scripts/_backup-momo-fix-${Date.now()}.json`;
writeFileSync(bf, JSON.stringify(backup, null, 2));
console.log(`\nbackup: ${bf}`);

if (dup.length) {
  const { rowCount } = await c.query(`delete from tb_forwarder where id = any($1::text[]) and fstatus in ('1','2','3','4')`, [dup.map(r=>r.id)]);
