...
// 2026-07-20 — owner dropped the latest 9 MOMO packing lists on the Mac Desktop.
// WHITELIST only those 9 (the Desktop also holds huge TTW *T.xlsx files in a
// DIFFERENT format — must never be parsed by this MOMO parser).
const PACKING_DIR = "/Users/dev/Desktop";
const FILE_WHITELIST = new Set([
  "GZS260720-1.xlsx", "GZE260718-1.xlsx", "GZS260718-1.xlsx", "GZE260716-1.xlsx",
  "GZS260715-1.xlsx", "GZS260712-1.xlsx", "GZS260706-1.xlsx", "GZS260705-1.xlsx",
  "GZE260714-1.xlsx",
]);
const BACKUP_PATH = "scripts/momo-packing-ingest-backup-2026-07-20.json";
...
// ─────────────────────────────────────────────────────────────────────────────
function selftest() {
  // 2026-07-20 — the 07-14 fixture (GZS260624-1.xlsx · 1782110296 = 6 boxes/82.5kg)
  // isn't on this machine; the parser itself is separately locked by
  // lib/admin/momo-packing-xlsx-parser.test.ts (test:unit · real fixtures).
  // Fall back to a structural check on the first whitelisted file: it must parse
  // with >0 aggregated bases and every aggregate carrying finite numbers.
  const fixture = path.join(PACKING_DIR, "GZS260624-1.xlsx");
  if (fs.existsSync(fixture)) {
    const p = parseMomoPackingXlsx(fs.readFileSync(fixture));
    ...
    const ok = target.parcelCount === 6 && Math.abs((target.totalWeight ?? 0) - 82.5) < 0.01;
    console.log(`\n== SELFTEST GZS260624-1 → ${ok ? "✅ PASS" : "❌ FAIL"}`);
    if (!ok) process.exit(1);
    return;
  }
  const first = [...FILE_WHITELIST].find((f) => fs.existsSync(path.join(PACKING_DIR, f)));
  if (!first) { console.error("SELFTEST FAIL: no whitelisted file found on Desktop"); process.exit(1); }
  const p = parseMomoPackingXlsx(fs.readFileSync(path.join(PACKING_DIR, first)));
  // phantom rows (re-captured section header/blank — no box count AND no weight)
  // are filtered in the main pipeline; mirror the same rule here.
  const phantom = (a) =>
    !a.baseTracking || a.baseTracking === "Tracking" ||
    (a.parcelCount == null && a.totalWeight == null && a.totalCbm == null);
  const real = p.aggregated.filter((a) => !phantom(a));
  const bad = real.filter((a) =>
    !a.baseTracking || !Number.isFinite(a.parcelCount ?? NaN) ||
    (a.totalWeight != null && !Number.isFinite(a.totalWeight)) ||
    (a.totalCbm != null && !Number.isFinite(a.totalCbm)));
  console.log(`\n== SELFTEST ${first} == container=${p.container} · bases=${real.length} (+${p.aggregated.length - real.length} phantom) · bad=${bad.length} · warnings=${JSON.stringify(p.warnings)}`);
  for (const b of bad) console.log(`  BAD: ${JSON.stringify({ base: b.baseTracking, boxes: b.parcelCount, wt: b.totalWeight, cbm: b.totalCbm })}`);
  if (real.length === 0 || bad.length > 0) { console.error("SELFTEST FAIL"); process.exit(1); }
...
async function main() {
  ...
  let files = fs.readdirSync(PACKING_DIR).filter((f) => FILE_WHITELIST.has(f));
  ...
  if (APPLY) {
    ...
    // build per-container stats for the gate stamp — EVERY successfully-parsed
    // container gets stamped (a fully-"ตรง" file still confirms the packing).
    const perContainerApplied = new Map();
    for (const pf of parsedFiles) {
      perContainerApplied.set(pf.container, {
        rows: 0, advanced: 0,
        trackings: pf.parsed.aggregated.length,
      });
    }
    for (const w of writePlan) {
      const st = perContainerApplied.get(w.cab);
      if (st) st.rows += 1;
    }
    // 2026-07-20 — stamp the billing packing-gate (lib/admin/packing-confirmed-cabs
    // treats a container as packing-confirmed when container_packing_reconcile has a
    // row OR momo_packing_upload does). The UI apply stamps this; the script must too,
    // else the วางบิล banner keeps saying "ตู้ยังไม่อัพ packing list".
    for (const [cab, st] of perContainerApplied) {
      await c.query(
        `insert into container_packing_reconcile (container_no, reconciled_at, reconciled_by, rows_updated, boxes_short, advanced, tracking_count, source)
         values ($1, $2, 'script-ingest', $3, 0, $4, $5, 'momo-packing-ingest-2026-07-20')
         on conflict (container_no) do update set reconciled_at = excluded.reconciled_at,
           reconciled_by = excluded.reconciled_by, rows_updated = excluded.rows_updated,
           advanced = excluded.advanced, tracking_count = excluded.tracking_count, source = excluded.source`,
        [cab, nowIso, st.rows, st.advanced, st.trackings]
      );
    }
  }
