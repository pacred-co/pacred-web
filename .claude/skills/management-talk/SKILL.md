...
## Pacred-specific notes (channel reality check)

Pacred ทีมเล็ก · ไม่มี JIRA/Slack ภายใน · channels จริงๆ ที่ใช้:

| ช่อง | คนรับ | shape ที่ใช้ |
|---|---|---|
| **`docs/research/poom-save-point-*.md`** | ภูม + Claude session ถัดไป | "JIRA comment" shape (full structured · TL;DR + commits + state) |
| **`docs/learnings/*.md`** | Claude agent ทุกตัวต่อไป | concept + cross-link · ไม่ใช่ exec-style |
| **chat → ภูม** | ภูม (peer engineer) | engineer-to-engineer ตรงๆ · ไม่ต้อง translate |
| **report ส่งพี่ป๊อป (owner)** | พี่ป๊อป · engineering-savvy แต่ไม่อ่าน code | **management-talk เต็มรูป** · TL;DR + impact + คนรับผิดชอบ + next |
| **commit message** | git log reader (Claude/human) | tight imperative · feature(scope): what + why |

When triggered for Pacred: ถ้าไม่ระบุ channel ให้ default = **"report ส่งพี่ป๊อป" shape** (เพราะนั่นคือ leadership audience ของ Pacred). ถ้าจะส่งภูม → ไม่ต้อง translate · ใช้ engineer-to-engineer.
