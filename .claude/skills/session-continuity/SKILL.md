...
- 🔴 **Proof must be the ARTIFACT THE USER CONSUMES — not an intermediate signal** (2026-07-24, the most expensive miss so far). Verifying one step short is the same as not verifying:
  | claimed done from… | what the user actually gets | verdict |
  |---|---|---|
  | `<title>` tag exists in the file | the title the **browser** picks (the FIRST one) | ❌ wrong on 13 pages |
  | `.no-print` class is on the button | whether a **rule hides it** on paper | ❌ menus printed for months |
  | the page renders correctly on screen | the **PDF** that comes out | ❌ blank page after every page |
  Ask literally: *"what does the user hold at the end?"* — a PDF, a printed sheet, a downloaded file, a LINE message. **Verify THAT.** If the artifact is one you cannot produce yourself, say so and ask for it (the owner sending back a real PDF is what finally closed this).
- **Print work has its own gate** → run the `print-verify` skill. `@media print` is untouched by every other check.
...
- **When context is near-full OR at session close**, write a SAVE-POINT to BOTH `memory/` (one entry, the durable index) AND the CLAUDE.md top save-point block + the relevant `docs/research/*.md`. The save-point must contain: branch + HEAD, what shipped (with commits), what's IN-FLIGHT (file:line + the half-done state), the EXACT next command/step, owner decisions still open, and creds/gotchas (chat-only secrets stay chat-only). A good save-point lets a cold start continue in one read.
...
RESTATE the visible symptom  →  PROBE source/prod until you know the real cause
  ...
  →  update the SAVE-POINT (ledger + memory + CLAUDE.md)  →  next unit
```
