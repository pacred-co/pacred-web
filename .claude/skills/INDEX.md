...
Last reviewed: 2026-05-18 (+mobile-first-verify — the render-at-360/390px customer-surface mobile check; +branch-integrate-loop — the integrate→verify→distribute consolidation cycle; +qa-flow-simulator — agent-driven end-to-end flow verification) · 2026-05-19 (+legacy-fidelity-check — the D1 owner-mandate fidelity gate; +landing-conversion-audit — the pre-ads landing CRO + tracking check) · 2026-05-27 (+debug-mantra — reproduce → fail path → falsify → breadcrumb discipline; +management-talk — engineer-to-leadership channel translator) · 2026-07-24 (+print-verify — the paper/PDF gate: @media print is invisible to build/tsc/lint, so every other gate reports green while the printout is broken) · 2026-07-24 (+mockup-first — owner directive: render a real mockup with real tokens + real prod data and get sign-off BEFORE writing production code for any visual change; a look/colour decision is made by EYE, and tsc/lint/build green says nothing about it)

---

## The skill set (21 skills — all `.claude/skills/<name>/SKILL.md`)
...
| **mockup-first** | "ทำหน้าตาให้ดูก่อน" · "อยากเห็นภาพก่อน" · "แต่งสีให้ดู" · "ขอดูก่อนทำจริง" · **ก่อนเขียนโค้ดจริงของงานหน้าตาทุกชิ้น** (สี · gradient · หัวตาราง · การ์ด · ป้าย · ฟอนต์) | สร้างปรีวิว HTML เดี่ยวๆ ด้วย**โทเค็นสีจริง + ข้อมูล prod จริง** → render + screenshot → ให้ owner เคาะ → **แล้วค่อยแตะโค้ดแอป** (งานหน้าตาตัดสินด้วยตา ไม่ใช่ด้วย gate) |
| **print-verify** | "พิมพ์ไม่ออก" · "ตกกระดาษ" · "หน้าขาวเบิ้ล" · "ชื่อไฟล์ผิด" · "ต้อง save PDF ไปปริ้นข้างนอก" · before shipping any page with `window.print()`/PrintButton | Verify the PAPER, not the build — print rules loaded · 0 floating chrome (wide **and** narrow) · `<title>` the browser really uses · `@page` present · measured page count · gated docs blocked on the right side of the money event |
