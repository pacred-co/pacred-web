...
interface SearchBarProps {
  ...
  /** When `inline`, renders the full bar NON-sticky + always-expanded (no collapse/toggle) —
   *  used to place the bar inline after the services on the mobile home (ปอน 2026-06-21). */
  inline?: boolean;
}

export function SearchBar({ embedded = false, hideOnMobile = false, defaultCollapsed = false, inline = false }: SearchBarProps) {
  ...
  const rootClass = embedded
    ...
    : inline
      ? "block w-full bg-white dark:bg-surface border-b border-gray-100 dark:border-border"
