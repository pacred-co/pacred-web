...
export const GLOSSARY_DEFS = {
  ...
  wht_1pct_bill:
    "หัก ณ ที่จ่าย 1% — ลูกค้านิติบุคคลหักภาษีไว้ 1% นำส่งสรรพากรแทนเรา แล้วจ่ายเรา “ยอดสุทธิ” (= ยอดรวม − 1%). เราได้ 1% คืนเป็นเครดิตภาษีเมื่อลูกค้าส่งใบ 50 ทวิ มาให้ · ใบบิลจึงโชว์ ยอดรวม → หัก 1% → ยอดชำระสุทธิ",
  ...
  // ── การชำระ / สถานะเงิน (payment board · owner 2026-06-28) ─────────────
  /** จ่ายแล้ว/ยังไม่จ่าย — derived from fstatus. */
  pay_state:
    "จ่ายแล้ว/ยังไม่จ่าย — “ยังไม่จ่าย” = ออเดอร์อยู่สถานะรอชำระเงิน · “จ่ายแล้ว” = ผ่านขั้นชำระไปแล้ว (เตรียมส่ง/ส่งแล้ว)",
  /** ยอดค้างชำระ (outstanding to collect). */
  owed_amount:
    "ยอดค้างชำระ = เงินที่ลูกค้ายังไม่จ่าย (สูตร calcForwarderOutstanding · รวมส่วนลด + นิติ 1%) — เงินที่ต้องตามเก็บ",
  /** ตัดกระเป๋า ชำระ — settle from the customer's wallet balance. */
  settle_from_wallet:
    "ตัดกระเป๋า ชำระ = ตัดเงินจากกระเป๋าลูกค้า (ที่มียอดอยู่แล้ว) เพื่อชำระออเดอร์นี้ทันที — ต้องมียอดในกระเป๋าพอ ระบบจะมาร์คชำระ + บันทึกให้อัตโนมัติ",
  /** ชำระนอก+สลิป — customer paid by bank, admin records the slip. */
  settle_outside:
    "ชำระนอก+สลิป = ลูกค้าโอนเข้าบัญชีบริษัทเอง (ไม่ผ่านกระเป๋า) → แอดมินแนบสลิป + บันทึกการโอนแทนลูกค้า แล้วชำระออเดอร์ในขั้นตอนเดียว",
  /** เงินสด vs เครดิต. */
  cash_vs_credit:
    "เงินสด = จ่ายก่อนรับของ · เครดิต = ลูกค้าได้วงเงิน จ่ายทีหลังตามเทอม (7/15/30 วัน) — ระบบดูจากวงเงินเครดิตของลูกค้า (userCredit) + ยอดเครดิตบนออเดอร์ (fcredit)",

  // ── cockpit ผู้บริหาร (owner 2026-06-28) ──────────────────────────────
  /** margin %. */
  margin_pct:
    "มาร์จิ้น = กำไร ÷ ยอดขาย × 100 · เขียว ≥15% ดี · เหลือง 0-15% บาง · แดง <0 ขาดทุน",
  /** ต้นทุนผิดปกติ (cost anomaly quarantine). */
  cost_anomaly:
    "ต้นทุนผิดปกติ = ออเดอร์ที่ต้นทุนสูงเกินยอดขายผิดสังเกต (น่าจะกรอกผิด) — ระบบกันออกจากการคิดกำไร ไม่ให้แถวเดียวทำตัวเลขผู้บริหารเพี้ยน · กดเข้าไปแก้ต้นทุนให้ถูก",
  /** per-service P&L. */
  service_pnl:
    "ภาพรวมทุกบริการ = รายได้/ต้นทุน/กำไร แยกตามบริการ (ฝากสั่งซื้อ · โอนหยวน · ฝากนำเข้า · freight เหมาตู้/แชร์ตู้) + รวมทุกบริการ — ภาพที่ครบกว่า KPI ด้านบนที่นับเฉพาะฝากนำเข้า",
  /** งานค้างต่อแอดมิน (employee audit stuck). */
  admin_stuck:
    "งานค้าง = งานที่อยู่สเตจเดิมเกิน 7 วัน ต่อแอดมินที่รับผิดชอบ (เซลล์ + CS ของลูกค้า) — ไว้ตามเร่งคนที่เกี่ยวข้องกับสเตจนั้น",
} as const;
