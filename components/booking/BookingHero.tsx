...
const BG_OVERRIDES_MOBILE: Record<string, string> = {
  customs: "/images/bannermobile/clearacnebanner4.png",
};
const BG_OVERRIDES_DESKTOP: Record<string, string> = {
  customs: "/images/bannerdesktop/clearancedesktop4.png",
};
  ...
  return (
    <div className="relative overflow-hidden aspect-[768/340] md:aspect-auto md:h-[400px] flex flex-col items-center justify-center px-4 md:px-7 pb-[64px] md:pb-[96px] rounded-b-2xl md:rounded-b-3xl">
      ...
      <div className="relative z-10 max-w-[1000px] mx-auto text-center text-white">
        <h1 className="relative top-2 md:top-4 text-[20px] sm:text-[28px] md:text-[clamp(34px,4.2vw,56px)] font-black tracking-tight leading-[1.1] md:leading-[1.1] mb-1.5 md:mb-4 text-white [-webkit-text-stroke:5px_#7f1d1d] md:[-webkit-text-stroke:8px_#7f1d1d] [paint-order:stroke_fill] [text-shadow:0_3px_8px_rgba(0,0,0,0.85),0_6px_18px_rgba(0,0,0,0.6)]">
          {t.rich(keys.titleKey, {
            em: (chunks: ReactNode) => (
              <em className="text-yellow-300 not-italic text-[32px] sm:text-[42px] md:text-[clamp(50px,6.2vw,78px)] [-webkit-text-stroke:7px_#7f1d1d] md:[-webkit-text-stroke:11px_#7f1d1d]">
                {chunks}
              </em>
            ),
        ...
        <p className="mt-2 md:mt-6 text-[13px] sm:text-[18px] md:text-[22px] font-extrabold text-white [-webkit-text-stroke:3px_#7f1d1d] md:[-webkit-text-stroke:4px_#7f1d1d] [paint-order:stroke_fill] [text-shadow:0_2px_6px_rgba(0,0,0,0.8),0_4px_12px_rgba(0,0,0,0.55)] leading-tight md:leading-snug px-1">
          {t.rich(keys.subKey, {
            em: (chunks: ReactNode) => (
              <em className="not-italic text-white text-[28px] sm:text-[34px] md:text-[clamp(44px,5vw,66px)] font-black tracking-tight md:ml-2 relative top-[24px] md:top-[40px] [-webkit-text-stroke:5px_#7f1d1d] md:[-webkit-text-stroke:8px_#7f1d1d] [paint-order:stroke_fill] [text-shadow:0_3px_10px_rgba(0,0,0,0.85),0_6px_18px_rgba(0,0,0,0.55)]">
                {chunks}
              </em>
            ),
            hl: (chunks: ReactNode) => <span className="text-yellow-300 relative top-[24px] md:top-[40px] [em_&]:!top-0">{chunks}</span>,
          })}
        </p>
      </div>
    </div>
