...
/**
 * ป้าย "+N" ที่ชี้เมาส์แล้วกางรายการที่ซ่อนไว้ (owner 2026-07-21 "เอาเมาส์ไปชี้
 * แล้วจะเห็นรายการเพิ่มเติม").
 *
 * ⚠️ ใช้ `position: fixed` ไม่ใช่ absolute — ตารางที่เรียกใช้อยู่ใน `overflow-x: auto`
 * ซึ่งตาม CSS spec พอแกนหนึ่งเป็น auto อีกแกนถูกบังคับเป็น auto ด้วย → popover แบบ
 * absolute จะ**โดนตัดหาย**. fixed หลุดจาก overflow ของ ancestor ได้
 * (ตราบใดที่ไม่มี ancestor ตัวไหน transform — เคยพังทั้งระบบมาแล้ว ดู [[nextjs-16-quirks]]).
 *
 * เปิดด้วย hover **และ** focus → คีย์บอร์ด/จอสัมผัสก็เข้าถึงได้ ไม่ใช่ hover-only.
 */
export function MoreChips({ label, title, children }: { label: string; title?: string; children: ReactNode }) {
  ...
  const open = () => {
    ...
    if (!r) return;
    // กันล้นขอบขวา — ถ้าชิดขอบให้ดันกลับเข้ามา (แผงกว้างสุด 280)
    const left = Math.min(r.left, Math.max(8, window.innerWidth - 288));
    setPos({ left, top: r.bottom + 6 });
  };
  const close = () => setPos(null);

  // พิกัด fixed จะเพี้ยนทันทีที่หน้าเลื่อน/ย่อขยาย → ปิดทิ้งดีกว่าโชว์ผิดที่
  useEffect(() => {
    if (!pos) return;
  ...
  return (
    <span
      ...
      className="cursor-default rounded px-1 text-[11px] font-semibold text-muted underline decoration-dotted underline-offset-2 transition hover:bg-muted/15 hover:text-foreground focus:outline-none focus-visible:ring-2 focus-visible:ring-primary-300"
    >
      {label}
      {pos && (
        <span
          className="fixed z-[130] flex max-w-[280px] flex-wrap gap-1 rounded-lg border border-border bg-white p-2 shadow-lg dark:bg-surface"
