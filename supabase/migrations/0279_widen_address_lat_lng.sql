-- ⚠️ RENUMBERED 0278 → 0279 (เดฟ 2026-07-24): เลข 0278 ชนกับ 0278_customer_esign_stamp
--    ของอีกเครื่อง (Claude/Mac) ที่ apply prod ไปแล้วเหมือนกัน — 2 คนเขียน migration พร้อมกัน
--    คนละเครื่อง (docs/team.md §0.1 เตือนไว้: เช็ค `ls supabase/migrations | tail` ก่อนตั้งเลข).
--    ตัว schema ไม่ชนกันเลย (คนละตาราง) และ **ทั้งคู่ apply prod แล้ว** — เปลี่ยนแค่ชื่อไฟล์
--    ให้ ledger ไม่กำกวม · ไฟล์นี้ idempotent (ALTER TYPE ซ้ำได้) ถ้าต้องรันอีกก็ปลอดภัย.
