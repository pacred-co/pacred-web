...
| 56 | [0060_member_code_3digit.sql](0060_member_code_3digit.sql) | generate_member_code() PR00001→PR001 (min-3-digit) + profiles backfill | **launch fix** |
| 57 | [0061_money_idempotency_guards.sql](0061_money_idempotency_guards.sql) | wallet_transactions kind+='cost_adjustment' + 3 partial-unique guards (forwarder pay · freight payment · tax invoice) — money audit P0-1/P1-2/P1-4 | **launch fix** |
| 58 | [0062_rls_role_pin_money_pii.sql](0062_rls_role_pin_money_pii.sql) | role-pin every money/PII/order/pricing `*_admin_all` RLS policy to explicit role arrays + wallet_transactions DB-level audit trigger — W-1 / gap-schema-security S-1 keystone (closes the driver/warehouse direct-PostgREST money-write hole) | **launch fix** |
| 59 | [0063_wallet_freight_invoice_reference.sql](0063_wallet_freight_invoice_reference.sql) | wallet_transactions reference_type+='freight_invoice' + partial-unique guard on the freight-payment wallet slice — W-3 / gap-schema-security G-3 (freight wallet-pay now writes a real wallet debit instead of a free shipment) | **launch fix** |
| 60 | [0064_wallet_overdraw_guard.sql](0064_wallet_overdraw_guard.sql) | wallet_available_balance() fn + wallet_assert_no_overdraw() BEFORE-trigger — hard non-negative floor on customer pending main-bucket debits with FOR UPDATE row-lock — gap-customer H-1 / S-5 aggregate-pending overdraw | **launch fix** |
