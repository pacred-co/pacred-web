...
| 54 | [0054_commissions.sql](0054_commissions.sql) | commission ledger — commission_tiers + commission_accruals + commission_withdrawals + commission_withdrawal_items + admins.role += interpreter | **V-E8 commission** |
| 55 | [0055_broadcasts.sql](0055_broadcasts.sql) | broadcasts table + notifications.broadcast_id FK | **V-G3 broadcasts** |
| 56 | [0056_accounting_periods.sql](0056_accounting_periods.sql) | accounting_periods + period_close_event + period-freeze BEFORE-trigger on invoices/payments | **V-E9 closing** |
| 57 | [0057_customs_declarations.sql](0057_customs_declarations.sql) | customs_declarations + customs_declaration_lines + per-shipment partial-unique | **V-E11 customs-decl** |
| 58 | [0060_member_code_3digit.sql](0060_member_code_3digit.sql) | generate_member_code() PR00001→PR001 (min-3-digit) + profiles backfill | **launch fix** |
| 59 | [0061_money_idempotency_guards.sql](0061_money_idempotency_guards.sql) | wallet_transactions kind+='cost_adjustment' + 3 partial-unique guards (forwarder pay · freight payment · tax invoice) — money audit P0-1/P1-2/P1-4 | **launch fix** |
...
| 71 | [0084_booking_documents.sql](0084_booking_documents.sql) | documents.booking_id link + doc_type CHECK extended for the booking-flow attach-documents selector (closes BK-1 audit gap G1). *Renumbered from 0081 — commit `a248696`.* | **BK-1.5 G1** |
| 72 | [0085_tax_invoices_credit_note_for.sql](0085_tax_invoices_credit_note_for.sql) | tax_invoices.credit_note_for_id — bidirectional credit-note (ใบลดหนี้) link per ADR-0006 §7 / RD Code 86. *Renumbered from 0082.* | **R3 G2e-2** |
| 73 | [0086_work_item_messages.sql](0086_work_item_messages.sql) | work_item_messages + work_items.waiting_for — IC-1 internal per-job chat + status-visibility layer (pairs with 0080 work-board). *Renumbered from 0083.* | **IC-1 T1** |
...
> [pcs-data-migration.md §9](../../docs/runbook/pcs-data-migration.md)). ภูม
