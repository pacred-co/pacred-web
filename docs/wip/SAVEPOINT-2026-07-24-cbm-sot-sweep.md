...
## 🔴 2026-07-25 รอบ 4 — บั๊กที่ผมทำเอง: กดนำเข้าไม่ได้ทั้งหน้า (fimages NOT NULL)

- 🔴 owner: *"เอาเข้าไม่ได้ครับ ... พนักงานกดเข้าไม่ได้จะแก้ยังไงครับ"* — 3/3 แถวพัง
      `null value in column "fimages" violates not-null constraint`
- **ต้นเหตุ = โค้ดผมรอบ 3**: เขียน `fimages: null` เมื่อไม่มีรูปเพิ่ม แต่คอลัมน์เป็น
      **NOT NULL DEFAULT '[]'** → ทุกการนำเข้าตายพร้อมกัน · **tsc/lint/build/test เขียวหมด**
      (ไม่มี gate ไหนรู้จัก constraint ของ DB) = เห็นตอนพนักงานกดเท่านั้น
- [x] FIX: `fimages: JSON.stringify(stagingExtraImages)` (ได้ `[]` เมื่อไม่มีรูป = ตรง default)
- [x] **กวาดทั้ง class ไม่ใช่แค่ตัวที่พัง** — เทียบ 91 คอลัมน์ที่ INSERT เขียน กับ schema prod:
      คอลัมน์ไม่มีในตาราง 0 · NOT NULL ไม่มี default ที่ INSERT ไม่เขียน 0 · เขียน null ทับ
      NOT NULL เหลือ 0 (หลังแก้)
- [x] **`scripts/verify-momo-commit-insert-2026-07-25.ts`** — ยิง INSERT จริงลง prod
      **แล้ว ROLLBACK เสมอ** (0 แถวค้าง · verify แล้ว) ครอบ 7 เคสใช้งานจริง: ยังไม่มีตู้ ·
      มีตู้+กำลังส่งมาไทย · มีรูปเพิ่ม · Product/Rem ยาวสุด · ที่อยู่ว่าง (ลูกค้าใหม่) ·
      เลขแทรค 50 ตัว+PR 10 ตัว · ค่าตีลัง+ประเภทพิเศษ → **ผ่านหมด** + เคสที่ 8 (fimages=null)
      **พังตามคาด** = พิสูจน์ว่าเทสไม่หลอก
- [x] **ยามถาวร `lib/admin/momo-commit-insert-guard.test.ts`** (ลงทะเบียน test:unit แล้ว) —
      สแนปช็อต 88 คอลัมน์ NOT NULL ของ prod + สแกน INSERT ในโค้ด · **พิสูจน์แล้วว่าจับได้จริง**
      (ใส่บั๊กกลับ → แดงทันที · เอาออก → เขียว) · มี comment วิธีรีเฟรชสแนปช็อตเมื่อมี migration
- 📌 บทเรียน: **gate ที่มี ไม่มีตัวไหนเห็น DB constraint** — ฟีเจอร์ที่แตะ INSERT/UPDATE บน
      ตาราง legacy ต้องมี rollback-test หรือ source-guard เสมอ ไม่งั้นรู้ตอนผู้ใช้กด

## ✅ 2026-07-26 — กระสอบ/ตีลังไม้ + ตู้กรอกได้บนแขนง "ปิดรอบ" + กรองขยะพัสดุขาด

- [x] **ตู้กรอกได้บนแขนง "⚠️ ปิดรอบแล้วแต่ยังไม่มีเลขตู้"** (owner: CS/DOC รู้เลขตู้จาก
      WeChat ก่อน MOMO ส่ง) — ✎ ผ่าน cabinetWriteGuard · **MOMO cid จริงมาแล้วชนะเสมอ ·
      sync ยึด key เดิม = เบิ้ลแถวไม่ได้** (ตอบข้อกังวล "MOMO ยิงดันสถานะแล้วเบิ้ลซ้ำซ้อน":
      propagate เขียนตู้เฉพาะช่องว่าง/placeholder · ไม่สร้างแถวใหม่)
- [x] **คอลัมน์ "กระสอบ" แยก** (owner สั่งซ้ำ) — CBX + จำนวน "N แทรค · N PR" ในกระสอบ
      เดียวกัน (prod ยืนยันเคสจริง: CBX260717-EK13 = 14 แทรค/9 PR) + **คอลัมน์ "ตีลังไม้"**
      = เลข PKF จาก raw.wooden_info (JSON string ต้อง parse) + รูปลังกดดูได้ + kg/คิว tooltip
- [x] **กรองขยะออกจาก "พัสดุที่ MOMO API ไม่ส่ง"** — เดิมโชว์ 4 รายการ = ขยะทั้งหมด:
      3 เลขกระสอบ CBX (แถว subtotal ของไฟล์ packing) + 1 แถวหัวตารางไฟล์ ("Tracking"/"Code")
      → กรอง sack + ไม่มีตัวเลข → เหลือของจริง 0 = **ตอนนี้ไม่มีพัสดุตกหล่นจริง** (กลไก
      ดึงเข้าระบบใช้ได้จริง — ตัวเดียวกับที่กู้ ฿294k · แค่ลิสต์เคยโชว์ขยะ)
- [x] **audit กระสอบทั้งระบบ (คำถาม owner "MOMO แยกยังไง เก็บตังยังไง")**:
      · MOMO เก็บกระสอบเป็น field ต่อพัสดุ (momo_sack_no · 155 แถว/24 กระสอบ) ·
        feed sack_info แยก = ตาย (0 แถวตลอด)
      · **บิล MOMO ไม่เคยมีบรรทัดเป็นเลขกระสอบ (0)** — เก็บเงินรายแทรคเสมอ → หลาย PR
        ในกระสอบเดียวเงินไม่ปนกัน (154 แถว/43 PR แยกต่อ userid ถูกต้อง)
      · เลขกระสอบหลุดเข้า ftrackingchn/fcabinetnumber = **0** (cabinetWriteGuard เอาอยู่)
      · หน้า detail ตู้มี tier ตู้⊃กระสอบ อยู่แล้ว (2026-07-20) — รอบนี้เพิ่มฝั่งตารางนำเข้า
- gate: tsc 0 · lint 0 · test:unit เต็ม 0 · build 0
...
## ✅ 2026-07-26 รอบ 3 — แท็บโกดัง + ตัดของตายทั้งหมด (owner "เหมือนคนละเว็บไซต์")

- [x] **NEW `components/admin/warehouse-workspace-nav.tsx`** = แถบเดียวใช้ **7 หน้า**:
      breadcrumb (Admin › ฝากนำเข้า › โกดัง › หน้านี้) + **แท็บโกดัง 🚢 กวางโจว(MOMO) /
      🧾 อี้อู(TTW)** + ปุ่มเครื่องมือของโกดังนั้น (หน้าปัจจุบันไฮไลต์) + **↩ ทางกลับ**
      (รายการฝากนำเข้า · รายงานตู้) — เดิมแต่ละหน้ามีแถบของตัวเองคนละแบบ = "เหมือนคนละเว็บ"
      · กวางโจว: ตรวจ+นำเข้า · Sync · packing · manual · บิลต้นทุน(gated) · MOMO Live
      · อี้อู: คีย์ใบส่งของ (CS) · แพคกิ้ง+ใส่ PR (DOC)
- [x] **ตอบ owner "CargoCenter ใช่ของเราหรอ ยังใช้อยู่ไหม" = ไม่ใช่/ไม่ได้ใช้** — prod
      tb_forwarder แยกโกดัง: MOMO **961 งาน** · TTW **32 งาน** · **CargoCenter/JMF/GOGO/
      TTP/momo-lcl = 0 งาน** (ไม่เคยมีเลย) + owner สั่งเอาออกตั้งแต่ 18/07 แต่แถบ tab
      หน้าอี้อูยังลิงก์อยู่ = **ทางเข้าหลอก** → 5 หน้าเป็น redirect เข้า hub (ไม่ลบไฟล์ =
      ลิงก์เก่า/บุ๊กมาร์กไม่ 404)
- [x] **คลังแชท WeChat ถอดทั้งระบบ** (owner "หนักรกเกะกะ") — หน้า `/admin/wechat-ops` ·
      panel "จีนว่าไง" บนหน้างาน + บนหน้าตู้ · `lib/admin/wechat-forwarder-context.ts` ·
      ลิงก์/เมนู/i18n keys
      ⚠️ **ไม่แตะ WeChat Pay** (ฝากโอนหยวน) · ช่องทางติดต่อองค์กร · WeChat ID แอดมิน = คนละเรื่อง
      ⚠️ **ตาราง `wechat_ops_message` (24,428 แถว) ยังอยู่** — ไม่ drop เอง (ลบ = กู้ไม่ได้ ·
      ไม่มีใครอ่านแล้ว = ไม่กินอะไร · รอ owner สั่งถ้าจะลบจริง)
- [x] **verify บนจอจริง** (login ที่ owner เปิดค้าง): hub/อี้อู/TTW = 200 + มีแท็บโกดังครบ ✓ ·
      `/admin/wechat-ops` = **404** ✓ · `/admin/api-forwarder-cn` กดแล้ว **เด้งเข้า hub จริง** ✓ ·
      ไม่มีคำว่า "คลังแชท" เหลือในทุกหน้า ✓ + screenshot หน้าอี้อูยืนยันธีมตรง MOMO
- gate: tsc 0 · lint 0 · i18n 0 · md 0 · test:unit เต็ม 0 · build 0
- 📌 **ไม่ push main** ตามกติกาใหม่ (§0k) — งานอยู่บน `dave-pacred2` · รอ owner สั่งขึ้น
