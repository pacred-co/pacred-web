...
- [x] **A. แพคกิ้งลิสต์เรียงคอลัมน์ตรงตารางใหญ่จริงๆ** (report-cnt/[fNo] dropdown)
...
- [x] **B. หัวแถว (summary) เพิ่ม tag ค่านำเข้า**: pill ปริมาตร/น้ำหนัก (uniq frefprice) ใน ค่านำเข้า cell +
      pill เรทขาย (uniq frefrate แดง) ใต้ ประเภท — เหมือนแถวเดี่ยวเป๊ะ.
- [x] **C. กล่อง/ลัง "-/1" → "0/1" ทุกจุด** (summary + แถวเดี่ยว + แพคกิ้งลิสต์): got null → 0.
- [x] **D. popup ทั้งระบบ — ต้องลอยกลางจอเสมอ** (ห้ามให้ user เลื่อนหา): root cause = fixed ภายใต้
...
- [x] **E. sidebar ย่อ/ขยาย บัง content ไม่ยอมพับ (บาง user เช่น admin_aom)** — ตรวจ pin/hover logic +
      z-index floating bar · ให้พับเมื่อคลิกนอก/เลือกได้แม้กาง.
- [x] **F. งานติดลบ /admin/forwarders/52197 งง** — root: ชิปเม้น 1782555393 ขายคิดตามน้ำหนัก (เรท 8/kg)
...
- [x] **G. gate รายการตรวจสอบ: ต้องตั้งที่อยู่จัดส่งก่อน** — adminReportCntAddCheck + client eligibility
      refuse แถวไม่มีที่อยู่ (ยกเว้น PCS รับเองโกดัง) กันบัคเงินค่าส่ง.
- [x] **H. เรทค่าขนส่งไทย auto (Flash/J&T เก็บตามจริง ไม่ใช่ 50)** — ตรวจ wiring resolveThShippingAutoPrice
      → ให้ fire ตอนวัดขนาด/เลือกขนส่ง (fill-when-empty · editable · ไม่แตะแถว billed).
- [x] **I. ปลดล็อค packing-gate ก่อนวางบิล (advisory)** + ย่อ tag 📦/⏳ เหลืออีโมจิ + tooltip.
