...
| **U1-3** | **Arrival→billing gate** — block mark-paid / pay-from-wallet for an arrived cargo job until container-no + final CBM confirmed | pairs with datanew **L-3**: PCS vs MOMO CBM disagree ~31% every container — bill off the MOMO closed-container figure, not the order-time estimate | Part W W-2 · MS §3.2 W-3w · datanew L-3 | M | ⏳ next |
...
**Status note (2026-05-17 night):** 5 of 8 U1 items shipped (U1-1, U1-2, U1-4, U1-5, U1-6). U1-3 is the next M-effort pickup. U1-7 awaits the L-0 API doc fix. U1-8 is env-only — waits for Sentry account / GTM container provisioning. Bonus: **U2-3 shipped** at `6681657` (`getFreightReceiptGate` wired against `freight_invoice_id` WHT entries).
...
| **U2-5** | **"sack" entity** — model the `CBX…-EK…` sack the legacy ops use; missing from the Pacred schema | datanew **L-4** — surfaces in IT-team chat container handling | datanew L-4 | M | ⏳ pending |
