...
7. **Commission engine, not a spreadsheet.** Per-deal + per-job + per-lead piece rates, the 50/50 SALE↔CS split, the team's drafted bonus model (§5.2) as configurable rules. Visible to each staff member. Directly fixes the Aom-style "ค่าคอมแปลกๆ" trust leak and the L-9 hardcoded-whitelist problem.
   > ⚠️ **TENSION — reconcile before the W6 commission tiers are confirmed (note 2026-06-10).** This "50/50 SALE↔CS split" is the **old HR-draft model** (พี่ป๊อป 2026-05-10, §5.2). It **conflicts** with the **CEO rule in [`global-trade-group-2026-06-04.md`](global-trade-group-2026-06-04.md) §8**: *"ใครเซอร์วิสคนนั้นได้ค่าคอม"* + flexible ownership (*CS ปิดงานเต็มคนเดียวได้ · sales ก็แย่งทำหน้าที่ CS ได้*) — i.e. commission follows **whoever actually services the deal**, which is NOT a fixed 50/50 when one person does both roles. 🔴 **The owner must reconcile the two before the W6 freight commission tiers (`business_config commission.freight_enabled` · mig 0167) are confirmed + the flag flipped** — the tier config encodes whichever rule wins. Do not hardcode 50/50 into the W6 tiers without an owner decision.
