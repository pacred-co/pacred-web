...
| 2 | LINE Notify (Apr 2025 EOL) | "ย้ายไป LINE OA push + สอนเซ็ท" | ✅ **Pacred infrastructure ALREADY wired tonight verified** — channel access token in `.env.local`, `sendLinePush()` at `lib/notifications/index.ts:125`, LIFF link flow at `/liff/link`, `/line-settings` for channel prefs, token VERIFIED via LINE bot/info probe (Pacred Shipping @pacred · 0/300 quota used). 3 steps left for ภูม: (a) add 4 LINE env vars to Vercel prod (5 min), (b) set `LINE_PUSH_BYPASS=false` Production scope only, (c) upgrade quota plan FREE→Light(฿500/mo · 5,000) or Standard(฿1,500/mo · 25,000). Note: webhook URL is ปอน's Cloudflare worker — independent of Pacred's push path, no need to migrate. Doc: [`docs/setup/line-oa-push-migration.md`](../setup/line-oa-push-migration.md) |
