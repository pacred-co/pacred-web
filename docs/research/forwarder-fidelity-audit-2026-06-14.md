...
5. **MOMO-LCL sack DEAD-READ** — `actions/admin/momo-lcl.ts` read `tb_tmp_forwarder_item_momo`. ✅ FIXED 2026-06-15 (P3). **Prod probe confirmed the deferral question:** `tb_tmp_forwarder_item_momo` = **0 rows** (dead) · `momo_import_tracks` = **77 rows** (live · Pacred MOMO sync lands here) · `momo_sack_infos` = 0. So the LCL verify tool returned 0 CBM/weight for EVERY sack. Repointed to `momo_import_tracks` with a data-verified column mapping (productid←`momo_cg_no` · producttracking←`momo_tracking_no` · cbm←`cbm` · weight←`weight_kg`, via PostgREST aliasing so downstream is unchanged). Display-only (no money write) · strict improvement (74/77 now match) · the 3 compound-`momo_cg_no` rows stay unmatched (same as the prior all-empty · no regression).
...
6. **Orphan `acc-payment.php`** (`/admin/accounting/payment`) — ✅ WIRED 2026-06-15 (P3 · §0d). NOT redundant: it reads the **LIVE** legacy ledger (`tb_wallet_hs` + `tb_payment` type=6 payStatus=2 · margin per txn), whereas `/admin/reports/yuan-profit` reads the rebuilt-twin `yuan_payments` and `/admin/yuan-payments` is the ops queue. Added as the 3rd `acc-*.php` sibling hub card (next to `acc-forwarder`→`/accounting/forwarder` + `acc-shop`→`/accounting/shop`).
