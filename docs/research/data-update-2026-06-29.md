...
## 🔴 CARGO COST (ลงข้อมูลเปิดใบกำกับ → ต้นทุนขาย 0669) — NOT ingested · PENDING owner (อย่ามั่ว)
47 cost rows (Σ ฿2.6M · per-customer cost-of-sale for ใบกำกับ/VAT) but **NO per-order link** (no tracking/cabinet · only PR · 35/47 PCS#/TTW-coded ≠ tb_forwarder · only 6 PR-coded fill 1:1). → can't attribute to tb_forwarder orders without guessing. **Recommendation: DON'T force it** — the per-order cargo cost is already canonical via MOMO (tb_forwarder.fcosttotalprice · 65 orders); this sheet = accounting cost-of-sale → belongs in the accounting/P&L lane (B1-B7 · ภพ.30), not tb_forwarder per-order. Owner to confirm or supply a tracking↔cost file. The ใบกำกับ-document files (~150 sheets) = archive (not a data table). เคลียร์ยอด ตั๋วพ่วง ตู้พี่ดำ = separate per-container reconcile (own phase).
