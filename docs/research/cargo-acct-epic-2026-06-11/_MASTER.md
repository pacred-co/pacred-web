...
5. **D-G2 — อากรขาเข้า + VAT-incl total** ✅ **BUILT + browser-verified** (Excel-killer · owner mechanism-first 2026-06-11 · commits `13cbd00c`+`0db474a4`). mig **0178** (`tb_forwarder.import_duty_pct/thb` · **applied DEV**, ⚠ **PROD pending**) + pure helper `lib/forwarder/import-duty-vat.ts` (31 tests · xlsx R9 satang-exact) + `setForwarderImportDuty` action (super/accounting/pricing · isolated) + `<ForwarderImportDutyEditor>` on forwarder `[fNo]` (อากร %/บาท · % auto-seeds baht · live ราคารวม VAT roll-up · confirm §0f). Browser-verified: อากร 100 → pre-VAT 100 → VAT 7.00 → VAT-incl 107.00. **Carryover:** apply 0178 to PROD; the VAT-incl → actual ใบกำกับ issuance = G1 (owner-blocked: PEAK GL + VAT-base). NEXT FREE migration = 0179.
