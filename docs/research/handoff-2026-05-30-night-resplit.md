...
1. **P0-22 — 4 cron retargets (START HERE).** `refresh-active-customers` still reads `profiles` (L87); `sales-daily-digest` still reads `wallet_transactions` (L49/69). Repoint all 4 (+`expire-probation`, `expire-driver-assignments`) → `tb_users.userActive` / `tb_wallet_hs` / `tb_admin` / `tb_forwarder_driver`. ~2-3h, all four. **Verified still dead.**
2. **P1-1/2 — task#41 forwarder list-bar.** `actions/admin/forwarders-bulk.ts::bulkUpdateStatus` still `from("forwarders")` (L203, rebuilt) — the faithful `adminBulkUpdateForwarderTbStatus` already exists; repoint + numeric `fStatus`. `bulkAssignDriver` → `tb_forwarder_driver`+`_item` batch. **Verified still dead.**
3. **P0-14 finish — status/cancel/note form in legacy-view.** `legacy-view.tsx` renders `MarkPaidTbForm` (pay) but NOT the `AdminServiceOrderUpdateForm` (status-flip/cancel→'6'/saveNote). Add it (the action already writes `tb_header_order` correctly). ~1h.
