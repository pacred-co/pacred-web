...
> 🔱 **Phase-B rework is now เดฟ+Claude-driven, wave by wave (2026-05-19).** To keep everyone working one direction + avoid collision: เดฟ + Claude execute the Phase-B customer-UI rework via spawned worktree agents that land on `dave`. **Wave 1 in flight** — the **customer 9-icon launchpad home** + the **customer order flow** (`/service-order`). **Do NOT rework those 2 screens in parallel.** ปอน's role now: pull `dave` often → **review + verify** each landed customer screen — the legacy PCS look + flow, mobile-first at 360/390px, TH+EN i18n parity — and fix the fidelity gaps the agents miss. The pickup list below is the Phase-B work menu — **ping เดฟ before taking a fresh screen** so each has exactly one owner.
