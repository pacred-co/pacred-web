...
4. **The `tb_*` schema migration** — Phase A applies the 117-table legacy schema as a new migration **`0084_pcs_legacy_schema.sql`** (`0081`-`0083` are taken by your booking / credit-note / chat batch — `0084` is the next free number). The `tb_*` tables coexist with the rebuilt `profiles`-era tables during the transition; nothing is dropped. The pre-D1 PCS-customer migration (`0067` · `actions/admin/pcs-migration.ts`) is **superseded** — don't extend it.

**Migration numbering:** taken — `0001`-`0083` (`0065` is an intentional gap). The Phase-A legacy-schema migration is **`0084`**; your Phase-B backend-rework migrations continue at `0085`+. Full deploy sequencing → [`runbook/pcs-data-migration.md`](../runbook/pcs-data-migration.md) §9.
