...
4. **The `tb_*` schema migration** — Phase A applies the 117-table legacy schema as a new migration **`0081_pcs_legacy_schema.sql`** (you renumbered your booking / credit-note / chat batch up to `0084`-`0086` in commit `a248696` to free `0081`-`0083` for exactly this — `0082`/`0083` stay open for legacy follow-up migrations). The `tb_*` tables coexist with the rebuilt `profiles`-era tables during the transition; nothing is dropped. The pre-D1 PCS-customer migration (`0067` · `actions/admin/pcs-migration.ts`) is **superseded** — don't extend it.
