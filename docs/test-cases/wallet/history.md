# Test Cases — `/wallet/history`

**ประวัติเดินบัญชีกระเป๋าเงิน**

> Source: `app/[locale]/(protected)/wallet/history/page.tsx` · Group: `(protected)`
...
- ตรวจข้อมูลที่ดึงจาก DB แสดงครบ (ไม่ใช่ ฿0/ว่างทั้งที่ควรมีข้อมูล) — table: `admin_audit_log`, `profiles`, `slips`, `tb_credit`, `tb_users`, `tb_wallet`, `tb_wallet_hs`, `wallet_transactions`

**ผลทดสอบ:**  `[ ] ✅ ผ่าน`   `[ ] ❌ ไม่ผ่าน`   `[ ] 🔧 ต้องแก้/ไม่ถูกต้อง`   `[ ] ⏭ ข้าม`

**หมายเหตุ / บั๊กที่เจอ:** _______________________________________________

### TC-04 · Feature — `customerCancelPendingWalletTx()`

**Action:** `actions/wallet` → `customerCancelPendingWalletTx`
...
### TC-05 · Feature — `getMyCredit()`

**Action:** `actions/credit` → `getMyCredit`
