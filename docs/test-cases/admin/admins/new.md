# Test Cases — `/admin/admins/new`

**เพิ่มผู้ดูแลใหม่**

> Source: `app/[locale]/(admin)/admin/admins/new/page.tsx` · Group: `(admin)`
...
### TC-05 · Feature — `adminCreateNew()`

**Action:** `actions/admin/admins` → `adminCreateNew`
...
**สังเกตเพิ่ม:** อาจ navigate → `/admin/admins?s=1${newId ? ` · list refresh อัตโนมัติ (`router.refresh`) · ส่งค่าว่าง/ผิด → error `invalid_input` (Zod) แสดง inline ไม่ submit
