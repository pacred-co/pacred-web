# Test Cases — `/admin/admins/[id]/edit`

**แก้ไขข้อมูลผู้ดูแล**

> Source: `app/[locale]/(admin)/admin/admins/[id]/edit/page.tsx` · Group: `(admin)`
...
- ตรวจข้อมูลที่ดึงจาก DB แสดงครบ (ไม่ใช่ ฿0/ว่างทั้งที่ควรมีข้อมูล) — table: `admin_contact_extras`, `admins`, `profiles`, `tb_users`
...
### TC-06 · Feature — `adminUpdateProfileFields()`

**Action:** `actions/admin/admins` → `adminUpdateProfileFields`
...
### TC-07 · Feature — `adminToggleActive()`

**Action:** `actions/admin/admins` → `adminToggleActive`
...
### TC-08 · Feature — `adminChangeRole()`

**Action:** `actions/admin/admins` → `adminChangeRole`
...
### TC-09 · Feature — `loadAdminForEdit()`

**Action:** `actions/admin/admins` → `loadAdminForEdit`
