# Test Cases — `/admin/leads`

**คิวโทรลูกค้าเย็น (cold-leads)**

> Source: `app/[locale]/(admin)/admin/leads/page.tsx` · Group: `(admin)`

## ⚙️ Preconditions

- ต้อง login เป็น **admin** ที่มี role: `super`, `sales_admin`, `sales`, `ops`
...
### TC-02 · สิทธิ์เข้าถึง — admin ที่ไม่มี role `super`/`sales_admin`/`sales`/`ops`
...
**ขั้นตอน:** login เป็น admin (super / sales_admin / sales / ops)
...
- ตรวจข้อมูลที่ดึงจาก DB แสดงครบ (ไม่ใช่ ฿0/ว่างทั้งที่ควรมีข้อมูล) — table: `admins`, `lead_call_log`, `tb_forwarder`, `tb_users`

**ผลทดสอบ:**  `[ ] ✅ ผ่าน`   `[ ] ❌ ไม่ผ่าน`   `[ ] 🔧 ต้องแก้/ไม่ถูกต้อง`   `[ ] ⏭ ข้าม`

**หมายเหตุ / บั๊กที่เจอ:** _______________________________________________

### TC-05 · Feature — `logLeadCall()`

**Action:** `actions/admin/leads` → `logLeadCall`
