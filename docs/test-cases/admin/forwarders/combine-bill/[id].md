# Test Cases — `/admin/forwarders/combine-bill/[id]`

**รายละเอียดบิลรวม**

> Source: `app/[locale]/(admin)/admin/forwarders/combine-bill/[id]/page.tsx` · Group: `(admin)`

## ⚙️ Preconditions

- ต้อง login เป็น **admin** ที่มี role: `super`, `ops`, `warehouse`, `accounting`
...
### TC-02 · สิทธิ์เข้าถึง — admin ที่ไม่มี role `super`/`ops`/`warehouse`/`accounting`
...
**ขั้นตอน:** login เป็น admin (super / ops / warehouse / accounting)
...
- แสดง component หลัก: `site`, `pacred-dialog`
- ตรวจข้อมูลที่ดึงจาก DB แสดงครบ (ไม่ใช่ ฿0/ว่างทั้งที่ควรมีข้อมูล) — table: `admins`, `tb_admin`, `tb_bill`, `tb_bill_item`, `tb_forwarder`
...
### TC-06 · Feature — `adminDeleteCombineBill()`

**Action:** `actions/admin/combine-bill` → `adminDeleteCombineBill`
...
**สังเกตเพิ่ม:** อาจ navigate → `/admin/forwarders/combine-bill` / `/admin/forwarders/combine-bill` · list refresh อัตโนมัติ (`router.refresh`) · ส่งค่าว่าง/ผิด → error `invalid_input` (Zod) แสดง inline ไม่ submit

**ผลทดสอบ:**  `[ ] ✅ ผ่าน`   `[ ] ❌ ไม่ผ่าน`   `[ ] 🔧 ต้องแก้/ไม่ถูกต้อง`   `[ ] ⏭ ข้าม`

**หมายเหตุ / บั๊กที่เจอ:** _______________________________________________

### TC-07 · Feature — `adminAddForwardersToBill()`

**Action:** `actions/admin/combine-bill` → `adminAddForwardersToBill`
...
### TC-08 · Feature — `adminRemoveForwarderFromBill()`

**Action:** `actions/admin/combine-bill` → `adminRemoveForwarderFromBill`
