# Test Cases — `/admin/forwarders/[fNo]/edit`

**แก้ไขมิติ/ค่าใช้จ่ายของออเดอร์นำเข้า**

> Source: `app/[locale]/(admin)/admin/forwarders/[fNo]/edit/page.tsx` · Group: `(admin)`
...
- ตรวจข้อมูลที่ดึงจาก DB แสดงครบ (ไม่ใช่ ฿0/ว่างทั้งที่ควรมีข้อมูล) — table: `admins`, `tb_admin`, `tb_forwarder`, `tb_forwarder_item`, `tb_rate_custom_cbm`, `tb_rate_custom_kg`, `tb_rate_g_cbm`, `tb_rate_g_kg`, `tb_rate_vip_cbm`, `tb_rate_vip_kg`, `tb_users`
...
### TC-06 · Feature — `adminUpdateForwarderDimensions()`

**Action:** `actions/admin/forwarders-edit` → `adminUpdateForwarderDimensions`

**ผลที่คาดหวัง:** ✏️ แก้ค่า → บันทึก → ค่าใหม่แสดงผลทันที (refresh). ค่าไม่ถูกต้อง → error inline

**สังเกตเพิ่ม:** อาจ navigate → `/admin/forwarders/${fNo}` · list refresh อัตโนมัติ (`router.refresh`) · ส่งค่าว่าง/ผิด → error `invalid_input` (Zod) แสดง inline ไม่ submit
