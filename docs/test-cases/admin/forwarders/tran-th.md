# Test Cases — `/admin/forwarders/tran-th`

**งานขนส่งในไทย (TH-transport batch)**

> Source: `app/[locale]/(admin)/admin/forwarders/tran-th/page.tsx` · Group: `(admin)`

## ⚙️ Preconditions

- ต้อง login เป็น **admin** ที่มี role: `super`, `accounting`, `warehouse`, `freight_sales`
...
### TC-02 · สิทธิ์เข้าถึง — admin ที่ไม่มี role `super`/`accounting`/`warehouse`/`freight_sales`
...
**ขั้นตอน:** login เป็น admin (super / accounting / warehouse / freight_sales)
...
- ตรวจข้อมูลที่ดึงจาก DB แสดงครบ (ไม่ใช่ ฿0/ว่างทั้งที่ควรมีข้อมูล) — table: `admins`, `tb_forwarder`, `tb_forwarder_tran_th_h`, `tb_forwarder_tran_th_sub`
