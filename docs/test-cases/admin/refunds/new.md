# Test Cases — `/admin/refunds/new`

**สร้างคำขอคืนเงิน**

> Source: `app/[locale]/(admin)/admin/refunds/new/page.tsx` · Group: `(admin)`
...
- ตรวจข้อมูลที่ดึงจาก DB แสดงครบ (ไม่ใช่ ฿0/ว่างทั้งที่ควรมีข้อมูล) — table: `admins`, `forwarders`, `profiles`, `refund_requests`, `service_orders`, `wallet_transactions`, `yuan_payments`

**ผลทดสอบ:**  `[ ] ✅ ผ่าน`   `[ ] ❌ ไม่ผ่าน`   `[ ] 🔧 ต้องแก้/ไม่ถูกต้อง`   `[ ] ⏭ ข้าม`

**หมายเหตุ / บั๊กที่เจอ:** _______________________________________________

### TC-05 · Feature — `adminCreateRefund()`

**Action:** `actions/admin/refunds` → `adminCreateRefund`
...
**สังเกตเพิ่ม:** อาจ navigate → `/admin/refunds/${res.data.id}` · ⚠️ ตรวจว่ามี **ขั้นยืนยัน** ก่อนทำจริง · ส่งค่าว่าง/ผิด → error `invalid_input` (Zod) แสดง inline ไม่ submit
