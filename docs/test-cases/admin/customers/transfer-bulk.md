# Test Cases — `/admin/customers/transfer-bulk`

**ย้ายเซลแบบกลุ่ม**

> Source: `app/[locale]/(admin)/admin/customers/transfer-bulk/page.tsx` · Group: `(admin)`

## ⚙️ Preconditions

- ต้อง login เป็น **admin** ที่มี role: `sales_admin`
...
### TC-02 · สิทธิ์เข้าถึง — admin ที่ไม่มี role `sales_admin`
...
**ขั้นตอน:** login เป็น admin (sales_admin)
...
- ตรวจข้อมูลที่ดึงจาก DB แสดงครบ (ไม่ใช่ ฿0/ว่างทั้งที่ควรมีข้อมูล) — table: `admins`, `profiles`

**ผลทดสอบ:**  `[ ] ✅ ผ่าน`   `[ ] ❌ ไม่ผ่าน`   `[ ] 🔧 ต้องแก้/ไม่ถูกต้อง`   `[ ] ⏭ ข้าม`

**หมายเหตุ / บั๊กที่เจอ:** _______________________________________________

### TC-05 · Feature — `bulkTransferCustomersToSalesRep()`

**Action:** `actions/admin/customer-transfer-bulk` → `bulkTransferCustomersToSalesRep`
