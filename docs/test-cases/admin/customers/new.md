# Test Cases — `/admin/customers/new`

**สร้างลูกค้าใหม่โดย admin (ไม่ต้อง self-register)**

> Source: `app/[locale]/(admin)/admin/customers/new/page.tsx` · Group: `(admin)`
...
- ตรวจข้อมูลที่ดึงจาก DB แสดงครบ (ไม่ใช่ ฿0/ว่างทั้งที่ควรมีข้อมูล) — table: `admins`, `profiles`, `tb_address`, `tb_address_main`, `tb_cash_back`, `tb_corporate`, `tb_forwarder`, `tb_header_order`, `tb_payment`, `tb_users`, `tb_wallet`, `tb_wallet_hs`

**ผลทดสอบ:**  `[ ] ✅ ผ่าน`   `[ ] ❌ ไม่ผ่าน`   `[ ] 🔧 ต้องแก้/ไม่ถูกต้อง`   `[ ] ⏭ ข้าม`

**หมายเหตุ / บั๊กที่เจอ:** _______________________________________________

### TC-05 · Feature — `adminCreateCustomer()`

**Action:** `actions/admin/customer-admin` → `adminCreateCustomer`
