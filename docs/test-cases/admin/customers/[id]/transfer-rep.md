# Test Cases — `/admin/customers/[id]/transfer-rep`

**ย้ายเซลผู้ดูแลลูกค้ารายนี้**

> Source: `app/[locale]/(admin)/admin/customers/[id]/transfer-rep/page.tsx` · Group: `(admin)`
...
- ตรวจข้อมูลที่ดึงจาก DB แสดงครบ (ไม่ใช่ ฿0/ว่างทั้งที่ควรมีข้อมูล) — table: `admin_contact_extras`, `admins`, `forwarders`, `profiles`, `service_orders`, `tb_users`, `yuan_payments`
...
### TC-05 · Feature — `adminTransferSalesRep()`

**Action:** `actions/admin/admins` → `adminTransferSalesRep`
...
**สังเกตเพิ่ม:** อาจ navigate → `/admin/customers/${customerId}` · list refresh อัตโนมัติ (`router.refresh`) · ส่งค่าว่าง/ผิด → error `invalid_input` (Zod) แสดง inline ไม่ submit
