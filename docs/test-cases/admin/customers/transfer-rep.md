# Test Cases — `/admin/customers/transfer-rep`

**ย้ายเซล (รายเดียว)**

> Source: `app/[locale]/(admin)/admin/customers/transfer-rep/page.tsx` · Group: `(admin)`

## ⚙️ Preconditions

- ต้อง login เป็น **admin** ที่มี role: `sales_admin`, `super`
...
### TC-02 · สิทธิ์เข้าถึง — admin ที่ไม่มี role `sales_admin`/`super`
...
**ขั้นตอน:** login เป็น admin (sales_admin / super)
...
- ตรวจข้อมูลที่ดึงจาก DB แสดงครบ (ไม่ใช่ ฿0/ว่างทั้งที่ควรมีข้อมูล) — table: `admin_contact_extras`, `admins`, `profiles`, `tb_admin`, `tb_users`

**ผลทดสอบ:**  `[ ] ✅ ผ่าน`   `[ ] ❌ ไม่ผ่าน`   `[ ] 🔧 ต้องแก้/ไม่ถูกต้อง`   `[ ] ⏭ ข้าม`

**หมายเหตุ / บั๊กที่เจอ:** _______________________________________________

### TC-05 · Feature — `adminBulkTransferSalesRepTb()`

**Action:** `actions/admin/admins` → `adminBulkTransferSalesRepTb`
