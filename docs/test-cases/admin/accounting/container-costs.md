# Test Cases — `/admin/accounting/container-costs`

**ต้นทุนตู้**

> Source: `app/[locale]/(admin)/admin/accounting/container-costs/page.tsx` · Group: `(admin)`
...
- ตรวจข้อมูลที่ดึงจาก DB แสดงครบ (ไม่ใช่ ฿0/ว่างทั้งที่ควรมีข้อมูล) — table: `admins`, `container_costs`

**ผลทดสอบ:**  `[ ] ✅ ผ่าน`   `[ ] ❌ ไม่ผ่าน`   `[ ] 🔧 ต้องแก้/ไม่ถูกต้อง`   `[ ] ⏭ ข้าม`

**หมายเหตุ / บั๊กที่เจอ:** _______________________________________________

### TC-05 · Feature — `adminCreateContainerCost()`

**Action:** `actions/admin/container-costs` → `adminCreateContainerCost`
...
### TC-06 · Feature — `adminArchiveContainerCost()`

**Action:** `actions/admin/container-costs` → `adminArchiveContainerCost`
