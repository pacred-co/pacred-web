# Test Cases — `/admin/accounting/periods/[period_yyyymm]`

**รายละเอียดงวดบัญชี**

> Source: `app/[locale]/(admin)/admin/accounting/periods/[period_yyyymm]/page.tsx` · Group: `(admin)`

## ⚙️ Preconditions

- ต้อง login เป็น **admin** ที่มี role: `super`, `accounting`, `ops`
- ต้องมีข้อมูลทดสอบสำหรับ param: `[period_yyyymm]` (ใช้ ID ที่มีจริงในระบบ)
...
- ตรวจข้อมูลที่ดึงจาก DB แสดงครบ (ไม่ใช่ ฿0/ว่างทั้งที่ควรมีข้อมูล) — table: `accounting_periods`, `admins`, `freight_invoice_payments`, `freight_invoices`, `period_close_event`, `tax_invoices`, `wallet_transactions`
...
### TC-06 · Feature — `adminMarkPeriodClosing()`

**Action:** `actions/admin/accounting-periods` → `adminMarkPeriodClosing`
...
### TC-07 · Feature — `adminClosePeriod()`

**Action:** `actions/admin/accounting-periods` → `adminClosePeriod`
...
### TC-08 · Feature — `adminReopenPeriod()`

**Action:** `actions/admin/accounting-periods` → `adminReopenPeriod`
