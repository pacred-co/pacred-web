# Test Cases — `/admin/barcode/driver/prepare`

**บาร์โค้ดคนขับ — เตรียมส่ง**

> Source: `app/[locale]/(admin)/admin/barcode/driver/prepare/page.tsx` · Group: `(admin)`

## ⚙️ Preconditions

- ต้อง login เป็น **admin** ที่มี role: `super`, `ops`, `warehouse`, `driver`
...
### TC-02 · สิทธิ์เข้าถึง — admin ที่ไม่มี role `super`/`ops`/`warehouse`/`driver`
...
**ขั้นตอน:** login เป็น admin (super / ops / warehouse / driver)
...
- แสดง component หลัก: `scanner-input`, `top-menu-barcode`
- ตรวจข้อมูลที่ดึงจาก DB แสดงครบ (ไม่ใช่ ฿0/ว่างทั้งที่ควรมีข้อมูล) — table: `admins`
