# Test Cases — `/admin/crm`

**CRM omni-inbox + customer-360 + routing**

> Source: `app/[locale]/(admin)/admin/crm/page.tsx` · Group: `(admin)`

## ⚙️ Preconditions

- ต้อง login เป็น **admin** ที่มี role: `super`, `manager`, `sales_admin`, `sales`, `ops`
...
### TC-02 · สิทธิ์เข้าถึง — admin ที่ไม่มี role `super`/`manager`/`sales_admin`/`sales`/`ops`
...
**ขั้นตอน:** login เป็น admin (super / manager / sales_admin / sales / ops)
...
- ตรวจข้อมูลที่ดึงจาก DB แสดงครบ (ไม่ใช่ ฿0/ว่างทั้งที่ควรมีข้อมูล) — table: `Podeng_customers_line`, `admin_contact_extras`, `admins`, `freight_quote`, `lead_call_log`, `tb_admin`, `tb_forwarder`, `tb_users`, `tb_wallet`

**ผลทดสอบ:**  `[ ] ✅ ผ่าน`   `[ ] ❌ ไม่ผ่าน`   `[ ] 🔧 ต้องแก้/ไม่ถูกต้อง`   `[ ] ⏭ ข้าม`

**หมายเหตุ / บั๊กที่เจอ:** _______________________________________________

### TC-05 · Feature — `setCustomerSalesRep()`

**Action:** `actions/admin/crm` → `setCustomerSalesRep`

**ผลที่คาดหวัง:** ✏️ แก้ค่า → บันทึก → ค่าใหม่แสดงผลทันที (refresh). ค่าไม่ถูกต้อง → error inline

**สังเกตเพิ่ม:** อาจ navigate → `/admin/crm?${params.toString()}` · list refresh อัตโนมัติ (`router.refresh`)
