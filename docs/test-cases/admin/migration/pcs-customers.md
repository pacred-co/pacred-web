# Test Cases — `/admin/migration/pcs-customers`

**เครื่องมือ migrate ลูกค้า PCS เดิม**

> Source: `app/[locale]/(admin)/admin/migration/pcs-customers/page.tsx` · Group: `(admin)`
...
- ตรวจข้อมูลที่ดึงจาก DB แสดงครบ (ไม่ใช่ ฿0/ว่างทั้งที่ควรมีข้อมูล) — table: `admins`, `pcs_legacy_customers_staging`, `profiles`, `v_pcs_migration_status`

**ผลทดสอบ:**  `[ ] ✅ ผ่าน`   `[ ] ❌ ไม่ผ่าน`   `[ ] 🔧 ต้องแก้/ไม่ถูกต้อง`   `[ ] ⏭ ข้าม`

**หมายเหตุ / บั๊กที่เจอ:** _______________________________________________

### TC-05 · Feature — `adminBackfillPcsAuthUsers()`

**Action:** `actions/admin/pcs-migration` → `adminBackfillPcsAuthUsers`
