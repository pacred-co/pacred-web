# Test Cases — `/admin/reports/agent-payouts`

**รายงานจ่ายคอมตัวแทน**

> Source: `app/[locale]/(admin)/admin/reports/agent-payouts/page.tsx` · Group: `(admin)`
...
- แสดง component หลัก: `report-shell`, `csv-button`
- ตรวจข้อมูลที่ดึงจาก DB แสดงครบ (ไม่ใช่ ฿0/ว่างทั้งที่ควรมีข้อมูล) — table: `admins`, `tb_forwarder`, `tb_user_sales`, `tb_user_sales_admin_pay`, `tb_users`

**ผลทดสอบ:**  `[ ] ✅ ผ่าน`   `[ ] ❌ ไม่ผ่าน`   `[ ] 🔧 ต้องแก้/ไม่ถูกต้อง`   `[ ] ⏭ ข้าม`

**หมายเหตุ / บั๊กที่เจอ:** _______________________________________________

### TC-05 · Feature — `getAgentPayoutReport()`

**Action:** `actions/admin/reports-agent-payouts` → `getAgentPayoutReport`
...
**สังเกตเพิ่ม:** ⚠️ ตรวจว่ามี **ขั้นยืนยัน** ก่อนทำจริง
