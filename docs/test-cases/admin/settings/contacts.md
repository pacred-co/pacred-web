# Test Cases — `/admin/settings/contacts`

**ตั้งค่าข้อมูลติดต่อ**

> Source: `app/[locale]/(admin)/admin/settings/contacts/page.tsx` · Group: `(admin)`

## ⚙️ Preconditions

- ต้อง login เป็น **admin** ที่มี role: `super`, `accounting`, `sales_admin`
...
### TC-02 · สิทธิ์เข้าถึง — admin ที่ไม่มี role `super`/`accounting`/`sales_admin`
...
**ขั้นตอน:** login เป็น admin (super / accounting / sales_admin)
...
- ตรวจข้อมูลที่ดึงจาก DB แสดงครบ (ไม่ใช่ ฿0/ว่างทั้งที่ควรมีข้อมูล) — table: `admins`, `org_contacts`

**ผลทดสอบ:**  `[ ] ✅ ผ่าน`   `[ ] ❌ ไม่ผ่าน`   `[ ] 🔧 ต้องแก้/ไม่ถูกต้อง`   `[ ] ⏭ ข้าม`

**หมายเหตุ / บั๊กที่เจอ:** _______________________________________________

### TC-05 · Feature — `createOrgContact()`

**Action:** `actions/admin/org-contacts` → `createOrgContact`
...
### TC-06 · Feature — `updateOrgContact()`

**Action:** `actions/admin/org-contacts` → `updateOrgContact`
...
### TC-07 · Feature — `deleteOrgContact()`

**Action:** `actions/admin/org-contacts` → `deleteOrgContact`
