# Test Cases — `/admin/service-orders/cart`

**ตะกร้าฝากสั่งซื้อ (admin)**

> Source: `app/[locale]/(admin)/admin/service-orders/cart/page.tsx` · Group: `(admin)`

## ⚙️ Preconditions

- ต้อง login เป็น **admin** ที่มี role: `super`, `ops`, `sales_admin`
...
### TC-02 · สิทธิ์เข้าถึง — admin ที่ไม่มี role `super`/`ops`/`sales_admin`
...
**ขั้นตอน:** login เป็น admin (super / ops / sales_admin)
...
- ตรวจข้อมูลที่ดึงจาก DB แสดงครบ (ไม่ใช่ ฿0/ว่างทั้งที่ควรมีข้อมูล) — table: `admins`, `tb_admin`, `tb_cart`, `tb_co`, `tb_header_order`, `tb_order`, `tb_settings`, `tb_users`

**ผลทดสอบ:**  `[ ] ✅ ผ่าน`   `[ ] ❌ ไม่ผ่าน`   `[ ] 🔧 ต้องแก้/ไม่ถูกต้อง`   `[ ] ⏭ ข้าม`

**หมายเหตุ / บั๊กที่เจอ:** _______________________________________________

### TC-05 · Feature — `adminRemoveCartItem()`

**Action:** `actions/admin/cart` → `adminRemoveCartItem`
...
### TC-06 · Feature — `adminEditCartQty()`

**Action:** `actions/admin/cart` → `adminEditCartQty`
...
### TC-07 · Feature — `adminSubmitCartAsOrder()`

**Action:** `actions/admin/cart` → `adminSubmitCartAsOrder`
