...
- ตรวจข้อมูลที่ดึงจาก DB แสดงครบ (ไม่ใช่ ฿0/ว่างทั้งที่ควรมีข้อมูล) — table: `service_order_items`, `service_orders`, `tb_admin`, `tb_corporate`, `tb_forwarder`, `tb_header_order`, `tb_order`, `tb_users`
