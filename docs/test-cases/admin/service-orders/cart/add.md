# Test Cases — `/admin/service-orders/cart/add`

**เพิ่มสินค้าในตะกร้า (admin)**

> Source: `app/[locale]/(admin)/admin/service-orders/cart/add/page.tsx` · Group: `(admin)`
...
- ตรวจข้อมูลที่ดึงจาก DB แสดงครบ (ไม่ใช่ ฿0/ว่างทั้งที่ควรมีข้อมูล) — table: `admins`, `tb_admin`, `tb_cart`, `tb_header_order`, `tb_order`, `tb_settings`, `tb_users`

**ผลทดสอบ:**  `[ ] ✅ ผ่าน`   `[ ] ❌ ไม่ผ่าน`   `[ ] 🔧 ต้องแก้/ไม่ถูกต้อง`   `[ ] ⏭ ข้าม`

**หมายเหตุ / บั๊กที่เจอ:** _______________________________________________

### TC-05 · Feature — `adminAddItemToCart()`

**Action:** `actions/admin/cart` → `adminAddItemToCart`
...
**สังเกตเพิ่ม:** อาจ navigate → `/admin/service-orders/cart?userID=${encodeURIComponent(userid)}` · ส่งค่าว่าง/ผิด → error `invalid_input` (Zod) แสดง inline ไม่ submit
