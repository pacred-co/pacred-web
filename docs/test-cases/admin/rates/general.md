# Test Cases — `/admin/rates/general`

**เรททั่วไป (engine)**

> Source: `app/[locale]/(admin)/admin/rates/general/page.tsx` · Group: `(admin)`
...
- ตรวจข้อมูลที่ดึงจาก DB แสดงครบ (ไม่ใช่ ฿0/ว่างทั้งที่ควรมีข้อมูล) — table: `admins`, `tb_admin`, `tb_customrate_hs`, `tb_hs_rate_custom_cbm`, `tb_hs_rate_custom_kg`, `tb_rate_g_cbm`, `tb_rate_g_kg`, `tb_rate_vip_cbm`, `tb_rate_vip_kg`, `tb_users`

**ผลทดสอบ:**  `[ ] ✅ ผ่าน`   `[ ] ❌ ไม่ผ่าน`   `[ ] 🔧 ต้องแก้/ไม่ถูกต้อง`   `[ ] ⏭ ข้าม`

**หมายเหตุ / บั๊กที่เจอ:** _______________________________________________

### TC-05 · Feature — `adminUpdateVipRateCells()`

**Action:** `actions/admin/rate-edits` → `adminUpdateVipRateCells`
...
### TC-06 · Feature — `adminUpdateGeneralRateCells()`

**Action:** `actions/admin/rate-edits` → `adminUpdateGeneralRateCells`
