# Test Cases — `/admin/broadcasts/new`

**สร้าง broadcast**

> Source: `app/[locale]/(admin)/admin/broadcasts/new/page.tsx` · Group: `(admin)`
...
### TC-05 · Feature — `adminCreateBroadcast()`

**Action:** `actions/admin/broadcasts` → `adminCreateBroadcast`
...
**สังเกตเพิ่ม:** อาจ navigate → `/admin/broadcasts/${res.data!.id}` · ส่งค่าว่าง/ผิด → error `invalid_input` (Zod) แสดง inline ไม่ submit
