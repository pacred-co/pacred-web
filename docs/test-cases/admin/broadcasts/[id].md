# Test Cases — `/admin/broadcasts/[id]`

**รายละเอียด broadcast**

> Source: `app/[locale]/(admin)/admin/broadcasts/[id]/page.tsx` · Group: `(admin)`

## ⚙️ Preconditions

- ต้อง login เป็น **admin** ที่มี role: `super`, `sales_admin`
...
### TC-02 · สิทธิ์เข้าถึง — admin ที่ไม่มี role `super`/`sales_admin`
...
**ขั้นตอน:** login เป็น admin (super / sales_admin)
...
- ตรวจข้อมูลที่ดึงจาก DB แสดงครบ (ไม่ใช่ ฿0/ว่างทั้งที่ควรมีข้อมูล) — table: `admins`, `broadcasts`, `notifications`, `profiles`, `tb_admin`, `tb_notify`, `tb_notify_read`
...
### TC-06 · Feature — `adminDeleteNotify()`

**Action:** `actions/admin/broadcasts` → `adminDeleteNotify`
...
**สังเกตเพิ่ม:** อาจ navigate → `/admin/broadcasts` · list refresh อัตโนมัติ (`router.refresh`) · ส่งค่าว่าง/ผิด → error `invalid_input` (Zod) แสดง inline ไม่ submit
