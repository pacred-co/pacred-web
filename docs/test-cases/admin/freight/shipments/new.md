# Test Cases — `/admin/freight/shipments/new`

**สร้าง shipment freight**

> Source: `app/[locale]/(admin)/admin/freight/shipments/new/page.tsx` · Group: `(admin)`
...
- แสดง component หลัก: `customer-picker`
- ตรวจข้อมูลที่ดึงจาก DB แสดงครบ (ไม่ใช่ ฿0/ว่างทั้งที่ควรมีข้อมูล) — table: `admins`, `freight_invoices`, `freight_parties`, `freight_shipments`

**ผลทดสอบ:**  `[ ] ✅ ผ่าน`   `[ ] ❌ ไม่ผ่าน`   `[ ] 🔧 ต้องแก้/ไม่ถูกต้อง`   `[ ] ⏭ ข้าม`

**หมายเหตุ / บั๊กที่เจอ:** _______________________________________________

### TC-05 · Feature — `adminCreateFreightShipment()`

**Action:** `actions/admin/freight-shipments` → `adminCreateFreightShipment`
...
**สังเกตเพิ่ม:** อาจ navigate → `/admin/freight/shipments/${res.data!.id}` · ส่งค่าว่าง/ผิด → error `invalid_input` (Zod) แสดง inline ไม่ submit
