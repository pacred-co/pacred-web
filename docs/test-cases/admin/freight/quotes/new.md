# Test Cases — `/admin/freight/quotes/new`

**สร้างใบเสนอราคา freight**

> Source: `app/[locale]/(admin)/admin/freight/quotes/new/page.tsx` · Group: `(admin)`
...
- ตรวจข้อมูลที่ดึงจาก DB แสดงครบ (ไม่ใช่ ฿0/ว่างทั้งที่ควรมีข้อมูล) — table: `admins`, `freight_quote_items`, `freight_quotes`, `freight_shipments`

**ผลทดสอบ:**  `[ ] ✅ ผ่าน`   `[ ] ❌ ไม่ผ่าน`   `[ ] 🔧 ต้องแก้/ไม่ถูกต้อง`   `[ ] ⏭ ข้าม`

**หมายเหตุ / บั๊กที่เจอ:** _______________________________________________

### TC-05 · Feature — `adminCreateFreightQuote()`

**Action:** `actions/admin/freight-quotes` → `adminCreateFreightQuote`
...
**สังเกตเพิ่ม:** อาจ navigate → `/admin/freight/quotes/${res.data!.id}` · ส่งค่าว่าง/ผิด → error `invalid_input` (Zod) แสดง inline ไม่ submit
