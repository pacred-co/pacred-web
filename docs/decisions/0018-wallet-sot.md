...
3. **Admin APPROVE / REJECT (the slip-review + manual-entry path).**
   ...
   - **Reject a customer DEBIT (withdraw refund, status `1→3`):** UPDATE `tb_wallet_hs.status='3'` + stamp admin; UPDATE `tb_wallet.wallettotal += amount` (give the money back). **CORRECTED 2026-05-30 (implementation found the legacy truth):** legacy admin `wallet.php` L807-815 does a **balance-bump on the same `tb_wallet` row**, NOT a new `type='5'` row — the rejected withdraw row simply flips to `status='3'` and the held balance is restored (no separate refund ledger line). This earlier draft floated a `type='5'` row; the legacy code is the authority (D1 faithful), so the shipped impl bumps the balance. (The implementation also guards a double-refund race: the status flip `.select()`s and only refunds if it actually flipped — a concurrent loser returns `alreadyDone`, no second refund.) This was a P0 in audit because Pacred previously just flipped status with no refund at all.
