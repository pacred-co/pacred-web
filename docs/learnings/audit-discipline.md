...
### + Audit BEFORE building a spec item — the platform is ~80% built, so "build X" is often "wire/verify X" (2026-07-06 · พี่ป๊อป spec 7h run)
Owner handed a hand-written spec (6-status cargo lifecycle + China key-in + accounting) reading like a from-scratch brief. Reality: mapped to ~10 items, **2 were already fully built** — a grep-first pass caught both before a rebuild: (a) container status-ladder จีน↔ไทย = `buildContainerJourney` ALREADY wired into `report-cnt/[fNo]` via `<ContainerJourneyPanel>` (7-stage timeline, G4 2026-06-30); (b) the district `routeOrderOf` route-order the sticker sheet needed already existed inline in `drivers/new` (extracted it to a shared SOT instead of copying). **Rule: for every "build the spec's X", first `grep -rln` the concept across `lib/`/`app/`/`actions/` + check if a builder/helper exists-but-unwired (§0e dead builder) vs exists-and-rendered. The spec describes the DESIRED end-state, not the current gap — measure the gap first.** Corollary: when the safe/clearly-scoped items are done, STOP and flag the money/data-model/owner-detail items (MOMO round semantics · consolidated-sack model · CS pay-on-behalf) for owner input — guess-building them violates the spec's own "ห้ามเดา · ห้ามนอกกรอบ" and risks money.

### + Fidelity-audit AGENTS massively OVER-FLAG — verify every "gap" against current source before touching it (2026-07-08 · legacy PCS faithful-audit)
Owner: run the legacy PCS admin + go through every page + make Pacred faithful to 100% before handover. Spawned an Explore agent to gap-audit the forwarder/cargo core (legacy `forwarder.php`/`report-cnt.php` vs our `/admin/forwarders`). It returned a confident matrix: forwarder LIST "78% faithful" with **7 "TOP GAPS"** (missing tax-doc column, purchaser column, origin badge, print/car badges, per-row edit buttons, price components). **I opened the CURRENT `forwarders-table.tsx` (post-ภูม) and verified each: ALL 7 were already implemented or false** — `<TaxDocBadge>` column (L837), `<PurchaserCell>` (L1074), origin block1Label (L936), 📄1-4 print + ↑ขึ้นรถ/↓ลงรถ badges (L995-1044), per-row ดูข้อมูล/อัปเดต (L1497/1503), and "price" = `outstanding_thb` via `calcForwarderOutstanding` = exactly legacy's `calPriceForwarderMain` computed total (legacy showed no separate component columns either). The core is ~95% faithful, not 78% — prior waves + ภูม's fidelity passes already did it. **Why agents over-flag on fidelity: they read the legacy exhaustively but skim OUR side (a 1819-line table file), so they miss that the feature is already there → they invent a rebuild.** Lesson: an agent audit is a *legacy-feature enumerator*, NOT a trustworthy gap list. Verify every claimed ❌/⚠️ against the actual current Pacred file (grep the component/column) before building — else you rebuild what exists (or worse, collide with a teammate who just wrote it). **And when the platform is already ~90-95% faithful, a blind 180-page re-sweep mostly re-confirms faithfulness = wasted tokens → pivot to owner-guided (ask which specific pages THEY see as "จาด/ไม่เหมือน") + spot-check the least-audited areas.**
