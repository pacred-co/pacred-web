...
## [2026-07-06] Idempotency guards over a soft-cancelled doc + a UNIQUE(fid) index = re-issue-after-void silently fails

**Context:** ภูม flag (PROD · PR086). Customer changed บุคคล→นิติ; staff voided the ใบวางบิล + ใบเสร็จ, re-billed, marked paid — but NO new ใบเสร็จ was created (silently). `lib/admin/auto-issue-receipt.ts` + `lib/admin/forwarder-tax-invoice.ts`.

**Symptom:** mark-paid succeeds, no receipt appears, no error shown to staff (the caller treats `alreadyIssued` as a benign no-op).

**Root cause — TWO walls, both must fall:**
1. **Idempotency guard was cancelled-blind.** The guard queried `tb_receipt_item` by `fid` and blocked if ANY row existed — but `voidReceipts` (actions/admin/accounting-receipts.ts) flips ONLY the `tb_receipt` header `rstatus→'2'` and NEVER deletes the `tb_receipt_item` rows. So the cancelled receipt's items survive and the guard falsely returned `already_issued`.
2. **UNIQUE(fid) below the guard.** Even after fixing the guard to ignore cancelled receipts (`.neq('rstatus','2')`), `tb_receipt_item.fid` is UNIQUE (`idx_16914_fid` · mig 0082; the tax-invoice twin `tb_forwarder_tax_invoice_item.fid` UNIQUE · mig 0129). The surviving cancelled item still holds the fid, so the new item INSERT hits **23505 → the catch deletes the orphan header → still no receipt.** The failure just moved from the guard to the INSERT.

**Fix (both auto-issue modules):** (A) guard ignores cancelled owners (block only if a NON-cancelled receipt/invoice covers a fid — money-safe: never double-covers an active doc); (B) in the re-issue path, **FREE the fids** — `DELETE tb_receipt_item WHERE fid ∈ issueFids AND rid ∈ priorRids` (priorRids = the guard-confirmed CANCELLED receipts) BEFORE the INSERT. Scoped by BOTH fid AND cancelled-rid so an ACTIVE doc's item is never touched. Delete-then-insert keeps the ≤1-item-per-fid invariant that `.maybeSingle()` readers rely on. Under concurrency the UNIQUE(fid) still serializes → exactly one receipt (loser 23505s → header cleaned up), so no double money-doc.

**Why the adversarial money-review was load-bearing:** the guard-only fix passed tsc/eslint AND looked correct, but a Workflow refuter caught the UNIQUE(fid) second wall by reading the migrations — without it I'd have shipped a fix that STILL didn't issue the receipt. Proven live on DEV: stale item → insert-same-fid → 23505 → scoped-delete → insert → success → cleanup to baseline.

**Why this matters next time:** any "issue X once per Y" idempotency guard over a table that is **soft-cancelled** (status flip, not row delete) MUST exclude cancelled rows from the block — AND if the child/item table has a **UNIQUE on the business key**, the re-issue path must also FREE that key (delete the cancelled owner's item rows). Search for the guard pattern `.from('*_item').select().in('fid'|'.eq('hno'|'payment_id')` + a `neq('status','cancelled')`-less block. **Still latent (flagged, not fixed — dormant + no cancel path today):** `lib/admin/yuan-tax-invoice.ts` (keyed by payment_id) + `lib/admin/shop-tax-invoice.ts` (keyed by hno) have the same cancelled-blind guard AND a UNIQUE key; both use `.maybeSingle()` which would ERROR on the 2nd issue once ฝากโอน/ฝากสั่ง ใบกำกับ goes live (`tax_invoice.shop_yuan_enabled`) + gains a cancel path — whoever wires that cancel must apply the same guard+free-fid treatment.

**Cross-links:** `lib/admin/auto-issue-receipt.ts` (guard L~193 + free-step L~430) · `lib/admin/forwarder-tax-invoice.ts` (guard L~97 + free-step L~245) · `actions/admin/billing-run.ts` markBillingRunPaid (caller) + voidBillingRunInvoices §7b (voids paid bills) · rstatus enum '0'ร่าง/'1'ออกแล้ว/'2'ยกเลิก/'3'รอชำระ · AGENTS §0e/§0f.

## [2026-07-06] coid/adminIDSale are tb_users columns, NOT tb_forwarder — any new tb_forwarder SELECT that adds them → Postgres 42703

**Context:** Building the จ่ายเงินแทนลูกค้า (pay-users.php) faithful port. `getPayUserForwarderView` (actions/admin/pay-user-view.ts) added `coid, adminidsale` to its `tb_forwarder` SELECT to render the VIP/Sale badges — the search returned nothing on screen (no wallet card, no table, no error banner that my checks matched).

**Root cause:** `tb_forwarder` has NO `coid` / `adminidsale` columns. They live on **`tb_users` as `coID` / `adminIDSale`** (customer-level attributes). The query 400'd with **Postgres `42703` (undefined_column)**; the loader correctly returned `{ok:false, error:"db_error:42703"}`, but that string didn't match my pattern-checks so it looked like "nothing rendered". This is the SAME trap as the 2026-07-05 "coid งานหาย" dashboard bug (added coid/adminidsale/promoid to a shop select → columns not on tb_header_order → whole query died silently).

**Fix:** pull `coID`/`adminIDSale` in `loadPanel` (which already SELECTs tb_users) → thread the panel values into every forwarder row (all rows belong to one customer, so the tier/rep is identical per row). Removed the two columns from the `tb_forwarder` SELECT + raw type. Probe-verified via PostgREST 400-loop (scratchpad probe) that only those 2 columns were bad.

**Why this matters next time:** before adding a customer-attribute column (coid/adminIDSale/promoid/userCompany) to any `tb_forwarder` / `tb_header_order` / `tb_order` SELECT, confirm it's actually on that table — these live on `tb_users`. tsc/eslint/i18n ALL pass with a bad column name (it's a runtime SQL error). §0e/§0c: verify the query returns real rows (or probe the DB columns) before claiming a page works.

**Test-harness gotchas hit while browser-verifying (Claude-in-Chrome + Next dev):**
1. **React 19 controlled inputs ignore JS `nativeSetter + dispatch('input')`** — the DOM value shows but React state stays "". Use the `computer` tool's real keyboard `type` into a JS-`.focus()`'d element, then press `Return` (the input's onKeyDown→search) — real key events fire React's onChange.
2. **Substring button selectors click the wrong element** — `.find(b => /ค้นหา/.test(text))` matched "ค้นหารายการฝากนำเข้า" elsewhere in the DOM, not the controls "ค้นหา". Use exact text match or `input.parentElement.querySelector('button')`.
3. **Stale Turbopack chunk after an edit** — the page kept serving a 500'd bundle (an old server-only import) until a hard re-navigate; dev logs showed the OLD error while the file was already fixed. Re-navigate to force recompile before concluding a bug.
4. **CDP screenshots time out ("renderer frozen") while a server action is in-flight**, but `javascript_tool` DOM queries still work — read `documentElement.textContent` (captures print-only/hidden layouts that `innerText`/`get_page_text` deprioritize).

**Cross-links:** memory `migrated-customer-reset-login-bug` (tb_users vs auth split) · CLAUDE.md 2026-07-05 dashboard "coid งานหาย" · AGENTS §0e/§0c.

## [2026-07-08] MOMO box-split "แตกกล่องไม่ได้" (weight_mismatch) — momo_box_detail mixes per-piece vs box-total across a folded row's boxes

**Context:** ภูม flagged a prod MOMO cargo row (fwd 52167 · tracking 760234506976 · PR079) pulled from the Live-discovery queue. It came in FOLDED (`famount=1, famountcount='1'` = รวมกล่อง · 1 aggregate row for a 4-box tracking). Clicking "แตกกล่อง MOMO เป็น 4 แถว" returned **"น้ำหนักรวมของกล่องแยกไม่ตรงกับรายการรวม — แตกไม่ได้"** (the `weight_mismatch` guard).

**Symptom / question:** The split's money-neutral guard compares `Σ(momo_box_detail.weight_kg × quantity)` to `tb_forwarder.fweight`. On 52167 that Σ was **1901.5 kg** vs the row's **172.5 kg** (a 90% mismatch → correctly refused). Why 10× off?

**Root cause:** `momo_box_detail` stores weight/cbm with an **INCONSISTENT convention across a folded row's own boxes** — MOMO's own Live scrape reports the bare-base box PER-PIECE but the "-i" siblings as the box TOTAL. Verified: box `-2` had `weight_kg=133, cbm=0.429226, qty=14`; its dims `43×31×23 = 0.0307 m³/piece × 14 = 0.429` prove the stored cbm is the box **TOTAL**, while the base box's `cbm=0.0384` (`53×29×25`) is **per-piece**. The split plan (`split-box-rows-plan.ts`) uniformly does `stored × qty`, so a box with qty>1 that's already a total gets multiplied AGAIN → catastrophic over-count. The aggregate `fweight=172.5` was itself built from that mixed convention (`base 13×2 + 133 + 5 + 8.5 = 172.5` exactly), so **no uniform stored-based rule can reproduce it** → the guard blocks a split that is actually fine. The guard was doing its job (a naive split would 10× the bill); the DATA was untrustworthy.

**The reliable signal = DIMENSIONS.** `width×length×height` is per-piece on EVERY box, so `Σ(w×l×h/1e6 × qty)` reconciles the trusted aggregate `fvolume` EXACTLY (0.603468 on 52167).

**Fix (`split-box-rows-plan.ts` · money-neutral · human-button only):** when the stored weight/คิว fail to reconcile the aggregate AND `opts.allowPriced` (the "แตกกล่อง" button, never the unattended cron) AND the dims reconcile `fvolume`, **fall back to a dimensional allocation** — spread the trusted aggregate fweight/fvolume/ftotalprice proportionally by each box's dims-volume share (anchor index 0 absorbs the rounding remainder → Σ === aggregate to the satang). `famount` = each box's real qty (undoes the fold). For a PRICED row the customer's bill = the PRESERVED `ftotalprice` (byte-identical); per-box fweight is a dims-proportional estimate that staff can edit — the whole point of splitting. Proven on the real prod row: 4 rows, Σ฿=3017.34 EXACT, Σweight=172.5 (not 1901.5), จำนวน=[2,14,1,1].

**Why this matters next time:** (1) a money guard that "correctly refuses" can still be blocking a legitimate operation because the underlying DATA is inconsistent — fix the reconstruction, don't weaken the guard. (2) When two independent MOMO paths write the same column (Live `fillMomoBoxDetails` = per-piece · discovery/scrape can be box-total), the column carries a MIX — never trust a single stored metric; cross-check against an orthogonal signal (dims). (3) The safe way to unblock money-touching code under a deadline: scope the change to the human-triggered path where the bill is preserved, keep the automatic/cron path byte-unchanged, and verify with a **dry-run against the real prod row** (read-only) not just hand-typed test values.

**Cross-links:** `lib/integrations/momo-web/split-box-rows-plan.ts` (planBoxRowSplit dims fallback) + `.test.ts` (29 checks) · `lib/integrations/momo-web/live-parcel-metrics.ts` (the per-piece convention) · `lib/admin/momo-live-discovery-plan.ts` (the folded-aggregate discovery path) · AGENTS §0e (verify the data, not just the type).

## [2026-07-08] MOMO จำนวนกล่องไม่ตรง (famount stale=1 vs MOMO=2) — fill-when-empty never re-syncs + no famount editor

**Context:** ภูม prod 52186 (แทรค 888072313163 · PR148): MOMO Live shows quantity=2 but the detail shows "จำนวน : 1 กล่อง", and "แก้จำนวนกล่องก็ไม่ได้". Owner: "แค่ดึงมาให้ตรงมันก็จบ ทำไมต้องยุ่งยาก".

**Root cause (TWO):** (1) `commit-momo-row-core.ts` sets `famount = metrics.qty` at commit — but MOMO's quantity can change AFTER commit (this row committed at qty=1, MOMO later said 2). The propagate re-fill (`propagate-live-data.ts`) guards famount behind FILL-WHEN-EMPTY (`!(famount>0)`) AND the whole fill is gated on weight being empty — so a row that already has a weight + a non-zero famount is NEVER re-touched → the stale count sticks forever. (2) `EditAmountCountField` only edited `famountcount` (รวม/ไม่รวม), never `famount` itself → no way to correct the number.

**Fix:** (a) `adminUpdateForwarderAmountCount` + `EditAmountCountField` gained an editable `famount` number input. (b) `propagate-live-data.ts` now SYNCS famount to MOMO's real quantity (not fill-when-empty) for unbilled SINGLE-parcel (parcelCount===1) bare-base rows where famount≠quantity — so the count auto-tracks MOMO. Both are money-SAFE: `famount` is DISPLAY-ONLY, verified in NO bill/outstanding/debit formula (grep for `* famount` = 0 hits) — the folded `famountcount='1'` marker is what tells the bill NOT to multiply by it, and that marker is left untouched.
...
## [2026-07-10] report-cnt "ตู้ซ้ำ 2 แถว" — a MOMO placeholder BORROWS a real container it doesn't own (display bug, not data bug)

**Context:** After MOMO fixed their API + ภูม re-synced, /admin/report-cnt showed the SAME container twice — e.g. GZE260704-1 as one row (34 parcels · packing ✓) AND a second row labelled "MOMO PR20260701-EK01" (20 parcels · ยังไม่อัพ). Customer complaint.

**Symptom:** two grouped rows displaying the same real container number. Looks like a double-insert / dedup failure.

**Root cause (a per-tracking prod probe settled it — the decisive experiment):** a MOMO **routing batch** (`PR20260701-EK01`, matched by `isMomoRoutingPlaceholder`) is closed into SEVERAL real containers, **per-tracking + progressively**. `propagate.ts` correctly rewrites each parcel's `fcabinetnumber` placeholder → real `container_batch_no` the moment ITS OWN tracking gets assigned (ungated cabinet-write, verified: **0** placeholder rows platform-wide have an own-tracking real container = propagate never misses). So the parcels STILL under a placeholder are exactly the ones MOMO hasn't assigned (own-tracking `container_batch_no` = NULL). BUT `resolveMomoContainerInfo`/`foldMomoContainerInfo` folded the WHOLE batch's `momo_import_tracks` rows and took the **first non-null** `container_batch_no` — borrowed from a sibling tracking that had already moved OUT → the genuinely-pending parcels masqueraded as that real container → false duplicate. `momo_import_tracks` keeps ALL the batch's trackings (moved-out + pending) keyed by the same `momo_container_no`, which is the trap.

**Fix (display only · no prod data touched · `f1935091`):** `resolveMomoContainerInfo(admin, cabs, trackingsByCab?)` now queries `momo_import_tracks` by the `momo_tracking_no` of the parcels **still under** each placeholder (report-cnt already computes `tracksByCab`) → pending → NULL → no real container claimed → the row shows the placeholder + a "⏳ รอ MOMO ผูกเลขตู้จริง" label. Auto-heals: once MOMO closes the ตู้, propagate rewrites `fcabinetnumber` and the parcel joins the real row. Wired into list + CSV export + `[fNo]` detail; falls back to batch-wide when `trackingsByCab` is omitted (tests/other callers).

**Why this matters next time:** when a container/entity shows "duplicated" on a MOMO surface, first ask **display-bug vs data-bug** with a per-tracking probe: does each row STILL under the placeholder have its OWN key's real value in the source table? If NO (0) → it's a resolver over-reach (a fold borrowing a sibling's value), not a propagate/backfill problem — fix the resolver, don't mass-rewrite prod. A "SEA0x / PR…-EK01" cabinet is a routing batch, NOT a container; resolve it from its own current members only. Related: [[momo-api-endpoint-limits]] · the MOMO box-split learnings above.

## [2026-07-13] paydeposit='1' ไม่ใช่ marker "จ่ายแล้ว" กลางๆ — ที่ fstatus 6 มันตัดแถวออกจากคิวมอบงานคนขับ

**Context:** Fix D (credit-settle on `markBillingRunPaid`) — an agent draft cleared `fcredit` AND stamped `paydeposit='1'` "to block re-collect, same marker the advance-paid block uses". Caught in integration money-review before commit.

**The trap:** `tb_forwarder.paydeposit='1'` at `fstatus='6'` EXCLUDES the row from the dispatch-assignable queue — legacy `forwarder-driver.php` gates on `(paydeposit<>1 OR paydeposit IS NULL) AND fStatus='6'`, mirrored by `lib/admin/pending-dispatch.ts` (the SOT for "งานรอจัดรถ") and `/admin/drivers/new`. A credit customer who pays their ใบแจ้งหนี้ BEFORE the goods dispatch would have their order vanish from มอบงานคนขับ = paid-but-never-delivered, a brand-new "สถานะไม่เดิน" stuck state (the exact class the G6 flag on advance-paid rows already tracks, awaiting the owner's call).

**The rule:** credit settlement mirrors the canonical `actions/credit.ts` EXACTLY — clear `fcredit: ""` + stamp `fdateadminstatus`, NOTHING else. Re-collect is already blocked because `isCreditUnsettledEligible` requires `fcredit='1'` as its first check (`lib/forwarder/billing-eligibility.ts`); no second marker needed. Before writing `paydeposit`, ask which QUEUE reads it — it is a legacy overloaded gate column (like fstatus's two axes), not a free boolean.
...
## [2026-07-15] Money-doc SOT-drift, the "hollow account" gap, and juristic re-stamp (owner-driven)

Three compounding patterns from an all-day owner session (all 4 branches → `51201a7d`).

**1. N surfaces computing the same money independently WILL drift — route every doc through ONE SOT.** The pay-user "ใบสรุปรายการที่ต้องชำระเงิน" (the doc the customer pays from) hand-rolled its total by summing stored `tb_forwarder` columns → it DROPPED the เหมาๆ ฿100 (a **virtual flat fee**, not a stored column) and computed WHT on the wrong base → PDF ฿7,220.51 vs the modal's ฿7,319.51 for the same order. The fix is never "recompute here too" — it's **route the doc through the same SOT the modal/bill use** (`computeForwarderDebitBatch` for เหมาๆ/net, `computeBillWht` for WHT, `resolveReceiptFrozenTotals` for the receipt, `resolveBillingIdentity` for identity). How to FIND the drift: trace ONE real customer's docs (bill vs receipt vs summary) against each other and against what they actually paid — the mismatch is the drift. A 6-agent workflow audit (amounts / identity / detail+linkage dimensions in parallel → plan → fix → adversarial verify) found the real bugs; the "primary flow is correct" ones it cleared saved a rewrite.

**2. The COD (paymethod='2') domestic-leg double-charge — a rule that lived in 2 of 4 engines.** `outstanding.ts` (bill) and `forwarder-collect-total.ts` (self-pay) both skipped the domestic leg for COD (the courier collects `ftransportprice` at the door). But `computeForwarderDebitBatch` (ใบแจ้งหนี้/pay-user/receipt) and `auto-issue-receipt.ts` did NOT → COD orders were charged the domestic leg twice. Lesson: when a money rule exists, **grep every engine that computes the same total** and confirm the rule is in ALL of them — a rule in some-but-not-all is a silent per-case error. Fix threaded `paymethod` through the row type + every caller's SELECT; absent ⇒ prepaid ⇒ no regression.

**3. The "hollow account" — a modern-created staffer invisible to a legacy SOT.** Selectable sales reps come from `tb_admin` (adminStatusA='1' AND adminStatusSale='1' · `sales-roster.ts`), NOT the modern `admins.role`. `/admin/admins` create-staff mirrors into tb_admin via `ensureLegacyAdminRow` **best-effort** — when it silently fails, the staffer has no tb_admin row and is invisible to EVERY sales surface AND to the sales-team toggle list (which reads tb_admin only) → "2 หน้าไม่ลิงก์กัน". Fix both ends: data (create the tb_admin row + the `admin_contact_extras.legacy_admin_id` bridge) AND code (the list UNIONs hollow modern staff so they appear and can be toggled). Lesson: a best-effort mirror between a modern table and a legacy SOT needs a **reveal-the-unmirrored** path, or the gap is undiscoverable from the UI.

**4. Productize the recurring one-off.** I had run a per-customer script (PR002/PR079) to re-stamp docs when a customer upgraded to นิติ. The owner wanted STAFF to do it: `listCustomerJuristicDocs` + `adminRestampCustomerDocs` + `<JuristicDocRestampPanel>` (ติ๊ก+เลือกทั้งหมด+Apply · docs already matching show "ตรงแล้ว✓"). When you find yourself scripting the same money-identity fix for a 2nd customer, build the self-service panel — the reusable core (`adminSetBillingRunBuyerIdentity`) already existed. Money-safe because identity/หัวชื่อ is display-only; the WHT recomputes live on the invoice but the receipt pins to frozen totals so the collected amount never moves.

**5. Juristic WHT refund — verify pay==composite before crediting (the anti-PR075 guard).** A customer who paid the full gross then became นิติ over-paid 1% → credit it to their wallet (PR217: 273.20). But **guard: each pay must equal the order composite** — a pay ≠ composite means WHT may already be applied, and a blind 1% refund double-refunds. Compute from the real composite, never a stale/zeroed field (the PR075 ¥-as-THB lesson). Always dry-run + backup + idempotency-session-key on any wallet write.
