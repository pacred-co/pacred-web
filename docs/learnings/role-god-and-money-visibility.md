...
### 7a. `profiles.is_active=false` is NOT a reliable "retired staff" signal (2026-06-21 regression)

First cut of the พี่ป๊อป-dup fix excluded `profiles.is_active=false` profiles from the admins
list. That hid **ก๊อต (admin_got, a real active Ultra Admin)** — he carries `is_active=false`
yet holds an active grant (active count dropped 23→22). `profiles.is_active` is set by various
flows (and legacy data), NOT exclusively by identity soft-delete. The correct "stale duplicate"
signal is **BOTH no active grant AND `profiles.is_active=false`** (that's PR034 — a merged dup
with its grant already off). Apply the exclusion at the PERSON level after dedupe:
`!(personActive === false && profile.is_active === false)`. A genuinely-resigned staffer keeps
`is_active=true` (+ ended_at / off grant) → still shows in the ลาออก tab. Lesson: before
filtering a directory on a boolean flag, probe prod for who the filter would REMOVE — a single
wrongly-hidden active admin is "งานหาย".
