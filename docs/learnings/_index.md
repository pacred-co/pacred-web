...
| [`php-port-patterns.md`](php-port-patterns.md) | Legacy `pcscargo` port gotchas · MySQL→PostgreSQL data migration (NUL bytes break COPY · zero-dates vs NOT NULL · live-DB convert · pgloader path · MySQL case-insensitive collation → normalise join keys · `notnull` is a PG operator · Supabase load needs the DB password) | 2026-05-19 |
