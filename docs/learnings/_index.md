...
|---|---|---|
| [`ci-and-deploy-gotchas.md`](ci-and-deploy-gotchas.md) | GitHub Actions · Vercel · pnpm · env-audit · git pathspec literal-brackets · Node fetch timeouts (IPv4-first DNS) · build/verify green ≠ prod (next-start smoke gate) · worktree smoke needs `.env.local` copy + rebuild · stale-worktree phantom bugs (verify vs the live `dave` worktree) · spawned worktree-isolation agents branch from `origin/main` not your working branch · a next-start+curl smoke can't detect a dead database · Vercel may flip dpl_id TWICE per push (intermediate build first — verify the COMMIT SHA via `/status`, not just the deploy ID) · Audit doc descriptions can misname legacy files — read the legacy source in full BEFORE porting · `googleapis` package crashes Next 16 tsc with heap-OOM (3500+ .d.ts files in the namespace import — use `google-auth-library` + raw fetch instead) | 2026-05-27 |
...
| [`parallel-agent-sprints.md`](parallel-agent-sprints.md) | 4 patterns from Sprint-9..15 retrospective — pre-audit before spawn · stale-doc cost · migration-number collision recovery · worktree-leak prompt prevention · cherry-pick over merge when source branch is BEHIND (diff-stat LIES: a 2-commit teammate branch can show 43 file changes because it's missing other teammates' work; blind merge would revert it) | 2026-05-26 |
