...
| [`supabase-storage-bulk-upload.md`](supabase-storage-bulk-upload.md) | Backfill script pattern (service-role · `upsert:true` for small idempotent batches · self-contained `.env.local` loader) · ASCII vs Thai filename safety · **the local `pcscargo/member/` dev dump is NOT a full prod mirror — `images/forwarder/`, `images/cnt/`, and per-shipment cargo photos must be pulled from the live host before back-filling** | 2026-05-22 |
