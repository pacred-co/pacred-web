...
### 2026-07-06 (พี่ป๊อป spec 7h run · machine-lag) — the ZOMBIE-AGENT root cause + the 1-at-a-time cadence
- **The uncommitted-truncated-agent (above) has a ROOT CAUSE: the agent spawns its OWN sub-agents / a Monitor task to poll its own tsc, then STALLS waiting on a child that orphans.** This run's #8 (China receive-bill, 1st attempt) finished "waiting on the monitor" with an EMPTY worktree (no commit) — same failure the older entry describes, traced: the spawned agent had itself spawned a Monitor-until-loop to watch its build and never returned. Owner noticed the orphaned children ("เอเจ่นค้างอีกแล้ว บ่อยจัง"). **FIX (put in EVERY worktree-agent prompt): "Do ALL work YOURSELF — NO sub-agents. NO Monitor/background-polling tasks. Run tsc-check ONCE inline (be patient, ~25min under load — WAIT for its exit) + build ONCE. COMMIT as soon as gates pass."** After adding that line, every subsequent agent (redo #8, #5, monitor, sticker) committed cleanly. You cannot kill an orphaned grandchild agent (no task-id surfaces) → the owner has to click Clear → PREVENT it in the prompt.
- **On a lagging machine, run agents 1-AT-A-TIME, not fanned out.** Owner mid-run: "เครื่องเริ่มกระตุก เบาลง". One worktree agent → integrate FF → re-build in the trunk → push → next. Slower wall-clock but zero deadlock, zero machine-lag complaints, each integration is a clean save-point. Ultracode's fan-out is for a healthy machine; an explicit "เบาลง" overrides it.
- **The 2-min shell timeout kills a build that's only ~25s idle but minutes under load.** `next build` via Bash defaults to a 120s timeout → SIGTERM mid-compile on a loaded box (saw "Exit code 143" mid-build). Pass `timeout: 480000` for build/tsc under concurrent load, read the real `BUILD_EXIT` from a redirected log (never `| tail`).
