...
## [2026-06-22] The lowercase-tb_users 42703 class was platform-wide (16+ pages) — found by a LIVE-QUERY SWEEP, not the gate

**Context:** the customers-list `userimage` bug (above) was the tip of an iceberg. `tb_users` is **camelCase** on prod (`userID/userName/userLastName/userTel/userEmail/userStatus/userRegistered/userCompany/coID/shopUser/adminIDSale/userActive/userPicture`), but many admin surfaces JOINED it with **lowercase** names (`userid/username/...`) → every such query `42703`-errors and is swallowed (`const { data } = ...`) → blank names / permanently-empty reports / a cron that never runs its UPDATE.

**How it was found (the method that matters):** a Workflow that, per admin-page cluster, **RAN every read-`select()` live against the DB** (`sb.from(T).select("<exact cols>").limit(1)` → report any error) + static-checked every write-column against the real table columns. 378 read-queries probed → **16 confirmed broken**, each self-verifying (a live error IS the bug). `tsc`/`lint`/`build`/`pnpm verify` are ALL blind to this — column names are runtime strings. Route-smoke is blind too (the pages return 200, just empty). The script template + clusters live in the `pacred-live-query-sweep` workflow.

**The 16+ (all camelCase or embed/col fixes · b42308e0 → 2325627a):** customers list + service-order detail (userPicture); wallet/{add×2,list,[id]}; accounting/{payment,forwarder,shop,withdraw}; reports/{sales-group,user-all} (empty reports); reports/{containers-hs,hs-code-revenue} (no `container_hs_lines↔containers` FK + `containers.code`→`container_no`); dashboard payShop (`amount`→`amount_total` + dead `profiles!profile_id` embed); customers/transfer-bulk (500: `admins↔admin_contact_extras` embed has no FK → split to 2 queries); sales-payouts-tb (`date`→`fdate`); settings/notifications (`roles`→`role`); service-import tracking (juristic WHT 1%); earn-trigger (commission accrual); reports-attribution; service-orders-shop-workflow; **the refresh-active-customers cron (UPDATE all-lowercase → errored every night → NEVER marked a customer active)**; adminBulkApproveCustomers (wrote phantom `adminidupdate`/`userdateactive` → member-approve fully broken, 6,968 stuck).

**Fix patterns:** SELECT → use Supabase column **aliasing** `userid:userID` (keeps every read-site + TS type untouched). Filters/`.order`/UPDATE payloads → the real camelCase name (aliases don't apply to writes). A no-FK embed → split into 2 queries + merge in JS (mirror `customers/[id]/transfer-rep`). `tb_wallet/tb_forwarder/tb_header_order/tb_payment` are **genuinely lowercase** `userid` — only `tb_users` is camelCase; don't blanket-rename.
