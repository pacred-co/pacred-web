...
## 2026-06-23 — a `flex-1` nav menu WITHOUT `min-w-0` clips the rightmost controls at narrow-xl (the bug invisible on a dev monitor)

**Symptom:** ภูม's customer (on a Surface laptop) saw NO language switcher in the customer navbar; our 1900px dev monitors always showed it. Same code, same logged-in `/dashboard`, different machine → looked like a deploy-lag or role bug. It was neither.

**Root cause:** the navbar is a single flex row — `logo` + `social (shrink-0)` + `mega-menu (flex-1)` + `right-cluster (shrink-0 · cart/bell/user + LocaleSwitcher + ThemeToggle)`. The mega-menu was `flex-1` **without `min-w-0`**. A flex item defaults to `min-width: auto`, so it **won't shrink below its content's min-width** — and a nav menu of `whitespace-nowrap` items has a large min-content width. At narrow-xl (~1280–1500 CSS px) the row's total exceeded the viewport, the row overflowed, and with the page root on `overflow-x: clip` the **rightmost items (LocaleSwitcher, then ThemeToggle) clipped off the right edge**. `xl` was still active (the mega-menu showed), so it wasn't the mobile cluster taking over.

**Why only some machines:** the failure window is **1280–1500 CSS px**, which is exactly where laptops at OS display-scaling land (a 1920px Surface at 150% = 1280 CSS px; 2-in-1s at 200% ≈ 1368). A dev on a 1900px+ monitor at 100% never enters that window → the bug is **invisible in normal dev** and only real customers hit it. Repro by setting the browser/preview viewport to 1281–1450, not by trusting a maximized 1080p+ window.
