...
| **docs@pacred.co** | **CS + DOCS shared central ops inbox.** ⚠️ Per CEO brief 2026-06-10, the **DOC team** (not CS) talks to carriers (สายเรือ/รถ/แอร์) + customs (นายด่าน/นายตรวจ) + freight; **CS gets status FROM Doc** and does **not** talk to สายเรือ/carriers (CS may follow up a truck driver directly only once given the number). So this hub = Doc handles freight/carrier/customs comms · CS handles the customer-facing inquiry + pulls status from Doc · plus sales/pricing/accounting/shipping coordination. | Footer (paired with sales) · FAQ contact form · CS inquiry handler · `emailContactPair[1]` | `CONTACT.emailDocs` |
