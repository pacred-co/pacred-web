...
รับข้อความลูกค้าจาก LINE Official Account → verify ลายเซ็น → เก็บลง 4 table ใหม่ (migration `0125_line_oa_inbox.sql`). ไม่แตะ table เดิมใด ๆ.

| ส่วนประกอบ | ที่อยู่ |
|---|---|
| Migration (4 tables) | [`supabase/migrations/0125_line_oa_inbox.sql`](../../supabase/migrations/0125_line_oa_inbox.sql) |
...
2. เปิดไฟล์ [`supabase/migrations/0125_line_oa_inbox.sql`](../../supabase/migrations/0125_line_oa_inbox.sql) → copy ทั้งไฟล์ → paste → **Run**
