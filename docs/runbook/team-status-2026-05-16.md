...
**Tests:** 272 unit-test assertions all green (test:unit suite, +17 from new relative-time helper).
...
| **T-P4** | G2 tax invoice issuance per ADR-0006 — phases G2a-G2f | 14-19h | 🟡 **G2a ✅ + G2b ✅ + G2c ✅ + G2d ✅ + G2e-1 ✅** — schema (เดฟ `f2230ed`) + customer request + admin issuance + PDF + download route + cancel-issued (this batch). G2e-2 credit note (~1-2h, separate; only needed for actual money refunds — typo correction works via cancel + customer re-request) · G2f audit+test (~2-3h) remain |
