...
⚠️ **Migration count is now 19** — `0044`-`0057` + `0060`-`0064` (ภูม shipped `0053`-`0057`: freight-WHT · commission ledger · broadcasts · monthly-closing · customs-declaration — V-E3/E4/E8/E9/E11/G3 batch, all integrated into dave). ภูม applies all 19 to Supabase dev → prod; runbook [`poom-apply-migrations-2026-05-17.md`](poom-apply-migrations-2026-05-17.md) updated (verify queries through (9)). **Ordering matters:** the W-3/0064 code expects schema `0062`-`0064` in the DB — apply the migrations **before** the `dave→main` deploy, else freight wallet-pay + the overdraw floor break.

> 🔬 **Verification — code-level pass: 🟢 GO.** A read-only code-level verification audit traced all 5 launch-critical paths (auth · wallet/money · admin RLS · order/forwarder · tax/WHT) → **no launch-critical flow broken**; W-1/W-3/S-3/S-4 + migrations `0062`/`0063`/`0064`/`0053` confirmed landed correctly line-by-line. 3 findings, none a blocker — **F-1 fixed** (`0053` was missing from `migrations/README.md` apply-list); F-2 (`/admin` dashboard ungated for driver/warehouse — gap-admin H-2) + F-3 (`getFreightReceiptGate` no-op, redundant) = post-launch. Detail → [`../research/prelaunch-verification-2026-05-17.md`](../research/prelaunch-verification-2026-05-17.md). The **live** functional verification (walk the billing flow on the deployed site, after the 15 migrations are applied) remains the [`UPGRADE_PLAN.md`](../UPGRADE_PLAN.md) §0 gate before post-launch upgrade execution.
...
⚠️ **19 migrations (`0044`-`0057` + `0060`-`0064`) are in git but NOT yet applied to Supabase** — เดฟ/agent reviewed all 19 SQL files 2026-05-17 (sound · idempotent · no bugs). **ภูม owns applying them to dev + prod** — apply straight from `supabase/migrations/` in ascending number order; steps + verify block in [`poom-apply-migrations-2026-05-17.md`](poom-apply-migrations-2026-05-17.md).
...
### ก๊อต (Senior Advisor)
- 🔴 **Mon morning — OAuth dashboard config** (the one open launch item): Vercel `NEXT_PUBLIC_SITE_URL` (currently dead `v2.pacred.co`) + Supabase Site/Redirect URLs + Facebook app → Live + Google redirect URI. Steps → [`auth-launch-fixes-2026-05-17.md`](auth-launch-fixes-2026-05-17.md). Only blocks social login — phone+OTP works regardless.
- ⏳ Standby Monday launch + Sentry alert watch first 48h
...
- ✅ 2026-05-17: customs-clearance landing refresh + `[port]` detail pages · `/line` redirect route + GTM tracking on every LINE CTA · ad-landing polish (20 components)
- ⏳ Standby Mon morning for landing tweaks
- 🚀 Post-launch (keep autonomous): T-N1/T-N2 SEO + ad-landing playbook · theme-token fix on remaining hardcoded surfaces per [`pacred-info.md`](../pacred-info.md) §"Migration tracker"
...
**End of team-status 2026-05-17.** Next checkpoint: [`team-status-2026-05-18.md`](team-status-2026-05-18.md) — the D1 pivot + branch-consolidation snapshot (the project redirected on 2026-05-18; see ADR-0017).
