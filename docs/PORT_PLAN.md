...
### Priority 0 — Sprint 5 wrap-up (~5-7h, ทำก่อนสุด)

| # | Task | Est | Decision? | Description |
|---|---|---|---|---|
| **P-15** | Wire `sales-daily-digest` recipient/dispatch | 2-3h | No | TODO block ใน `app/api/cron/sales-daily-digest/route.ts:107-128` มีสเปคครบ. (a) migration `0025_profiles_notify_channels_daily_digest.sql` เพิ่ม `daily_digest` flag ใน `profiles.notify_channels` jsonb default false (b) loop admins where `role IN ('super','sales_admin')` AND `notify_channels.daily_digest = true` (c) call `sendNotification(adminId, {category:'sales_digest', title, body: formattedMessage})` per admin. **Acceptance:** local hit endpoint → see notifications row created per admin opt-in |
| **P-16** | Verify + enable `refresh-active-customers` schedule | 1h | ภูม decide D-18 | Recommended resolution: **คงทั้งคู่** — cron flips `is_active=true` (cheap once-a-day write); P-13 dashboard อ่าน flag + computes อะไรก็ได้ (real-time aggregate optional). Verify: (a) hit endpoint locally → confirm flipped count match expected (b) confirm forwarder enum exclusion ถูก (only `pending_payment` excluded) (c) production env: confirm Vercel cron picked up `0 1 * * *` schedule. Document decision in commit |
| **P-17** | Port `check-apprentice` admin half | 2-3h | No | Driver half (forwarder_driver) ทำใน P-18. Admin half: (a) migration `0026_employees_contract_end_date.sql` add `contract_end_date date null` to employees + index (b) new route `app/api/cron/expire-probation/route.ts` — sweep employees where `employee_type='probation' AND contract_end_date < now() AND is_active=true` → set `is_active=false` + audit log (c) vercel.json add `0 2 * * *` schedule (03:00 ICT). **Acceptance:** test seed an expired probation employee → run endpoint → verify deactivated + audit row |

### Priority 1 — Cargo port completeness (~16-22h, no decisions needed)

| # | Task | Est | Decision? | Description |
|---|---|---|---|---|
| **P-18** | `forwarder_driver` table + basic admin CRUD + driver expiry cron half | 4-6h | No | Schema migration `0027_forwarder_driver.sql` mirroring legacy `tb_forwarder_driver` (id, forwarder_id FK, profile_id FK driver, status enum 1=assigned/2=accepted/3=expired/4=completed, fdDate timestamptz, accepted_at, completed_at). Admin pages: `/admin/drivers` list + `/admin/drivers/[id]` detail. Then add second route `/api/cron/expire-driver-assignments/route.ts` — sweep `status=1 AND fdDate < now() - interval '17 hours'` → status=3 |
| **P-19** | CSV bulk import (`tb_csvimport`) port | 4-6h | No | Admin tool from PHP `import-excel.php` + `single-code-text-converter.php`. Migration `0028_csv_imports.sql` + admin page `/admin/csv-imports/{,upload,[id]}` — upload CSV → Supabase Storage `csv-imports/` bucket → server-side parse → preview rows → confirm import → write to target table (start with `forwarders` only — most common use case). Use `papaparse` npm or built-in parsing. **Acceptance:** upload sample CSV → preview 5 rows → confirm → forwarders inserted |
| **P-20** | Container HS code rates (`cnt-hs` + `hs-customrate` + `report-cnt`) port | 3-4h | No | Migration `0029_hs_codes_rates.sql` — `hs_codes` (code text PK, description text, default_duty_pct numeric) + `container_hs_lines` (container_id FK, hs_code FK, qty, weight_kg, value_thb). Admin pages `/admin/containers/[id]/hs` (line items entry) + `/admin/reports/containers-hs` (aggregated by hs_code). **Acceptance:** add 3 lines to container → see report group by HS code |
| **P-21** | Notification template system (DRY for sendNotification calls) | 3-4h | No | New `lib/notifications/templates.ts` exporting typed template builders: `salesDigest({date, totals})`, `forwarderStatusChange({fNo, oldStatus, newStatus})`, `walletDepositApproved({amount})`, etc. Each returns `NotifyPayload` with category/title/body filled. Refactor existing call sites (`actions/admin/forwarders.ts`, `actions/admin/wallet.ts`, etc.) to use templates instead of inline literals. Reduces drift + eases i18n later. **Acceptance:** ≥5 call sites refactored, TS clean |
...
**Sprint 6 estimate:** ~33-49h → 3-4 weeks part-time of self-directed work for ภูม

**Order recommendation:**
1. Day 1: P-15 + P-16 + P-17 (close cron work, ~5-7h)
2. Day 2: P-18 (driver table, ~4-6h)
3. Day 3: P-19 (CSV import, ~4-6h)
4. Day 4: P-20 + P-21 start (~6-8h)
5. Day 5: P-21 finish + P-24 tests start (~4-6h)
6. Day 6+: P-22 / P-23 / P-25 / P-26 / P-27 ในลำดับใดก็ได้

**Hand-off rule:** หลังจบ task → push branch + commit อัพเดท Part P snapshot ว่า P-XX done. ถ้าระหว่างทางเจอเรื่องที่ ภูม ไม่แน่ใจว่าควร decide เอง — flag เป็น commit doc proposal ใน `docs/decisions/` แล้ว ping เดฟใน LINE
...
13. ✅ **D-12** Rate limit abstraction — `lib/rate-limit.ts` with Upstash adapter (when `UPSTASH_REDIS_REST_URL`+`_TOKEN` set) + in-memory `Map` fallback (dev only). Pre-configured limits: signup 5/h/IP · login 10/h/IP · passwordReset 5/h/IP · contact 5/h/IP · generic 30/min. `checkRateLimit(name, key)` returns `{ ok:false, error:'rate_limit', retryAfterSeconds }` or `null` to continue. `getClientIp(req)` extracts from `x-forwarded-for`. Sliding-window via Upstash (fairer than fixed). Wiring to specific endpoints = follow-up task (won't auto-wire to avoid surprise UX). Env vars documented `.env.example` + `docs/env.md` §12. Note: complementary to existing OTP DB-backed limit in `actions/otp.ts` (3/h/phone via `otp_codes` table — that one doubles as audit trail)

### 🟡 REMAINING (Sprint 5 Days 3+)
14. 🔴 **D-7a** Set 3rd-party API env vars + verify endpoints (2-4h) — **blocked: need real credentials** (RCGroup URL, TAMIT URL, ThaiBulkSMS keys, PromptPay ID)
15. 🔴 **D-7b** LINE Messaging API setup (3-4h) — **blocked: need LINE Channel Access Token from Pacred OA**
16. 🟡 **D-11-activate** Get Sentry account + DSN → drop in Vercel env (15-30m) — **blocked: need Pacred owner to create Sentry account / authorize use**
17. 🟡 **D-12-activate** Create Upstash Redis DB + drop creds in Vercel env (15-30m) — **blocked: need Pacred owner to authorize**
18. 🟡 **D-12-wire** Wire `checkRateLimit` into actual endpoints (signup / login / contact / password reset) (1-2h) — could do now; deferred so callers can pick ergonomic UX response per endpoint
19. 🟡 **D-13** CAPTCHA on signup (hCaptcha invisible) (2-3h) — **blocked: need hCaptcha site key** (could scaffold ahead similar to D-11/D-12)
20. ⚪ **D-7c** Decision: Payment Gateway provider (with Pacred owner) → M2.1 design
...
**Sprint 5 Day 3 evening:** Sentry SDK scaffold + audit fix + rate-limit abstraction landed on dave (D-11 + D-12 ✅) — D-13 CAPTCHA next
...
- เดฟ: D-7a/b/c/d + D-13 = 5 items blocked on creds/owner decisions; D-11 + D-12 ✅ scaffolded (just need DSN/Upstash creds to activate); D-12-wire = 1-2h follow-up wiring task
...
1. **เดฟ → main:** push Day 3 evening merge ✅ done → ping ทีม + ก๊อต ว่า main updated + collab pattern §9 + self-directed mode §6 เปิดใช้
2. **ภูม:** Sprint 6 runway P-15 → P-27 (~33-49h) ใน Part O2 — **self-directed mode**, ลุยได้เลย ไม่ต้องรอ confirm. ลำดับแนะนำ: P-15 → P-16 → P-17 → P-18 → P-19 → P-20 → P-21 → P-24 (tests) → P-22/23/25/26/27 ในลำดับใดก็ได้
3. **ปอน:** Phase B L-5 polish (ส่งเดฟ confirm priority pages + style ก่อนเริ่ม) — แนะนำ home → import-china → china-shopping ตามลำดับ. หรือเริ่ม Phase D L-9b/c i18n polish ที่ self-directed ได้เลย
4. **ก๊อต:** review main batch · schedule Pacred owner call (D-7 + creds list)
5. **เดฟ:** ระหว่าง runway ภูม → focus blocked items (D-7a/b/c/d/D-11/12/13) เมื่อ creds เข้ามา + drop-in review งานน้อง
