...
### Priority 5 — Sprint 7 candidates (ภูม pick after follow-ups + Priority 2/3 wrap)

P-21 (notification templates) + P-22 (time attendance) + P-23 (meeting room) + P-24 (rate engine tests) + P-25 (Part N3 re-audit) + P-26 (service-order integration test) + P-27 (DPX ERP ADR draft) ยังคง pending — ดู Priority 2/3 ด้านบนสำหรับ spec

---

**Sprint 6 actual:** P-15 → P-20 done in ONE session (~16-22h work) + audit by เดฟ. Sprint 6.5 = 6 follow-ups (~2-3h) + Priority 2/3 (~17-25h) → ภูม keep grinding
...
1. **เดฟ:** ✅ pushed Day 3 evening merge `e9da976` → ping ทีม + ก๊อต ว่า main updated + Poom merged
2. **ปอน:** 🔴 fix L-pricing-fix (15m) — add 6 i18n keys + drop 4 orphan keys → push podeng → เดฟ merge
3. **ภูม:** Sprint 6 follow-ups (6 items, ~2-3h, self-directed) → Priority 2 P-21..P-24 → Priority 3 P-25..P-27. ลำดับแนะนำ: P-vercel-plan + P-20-followup-rls (5-15m เก็บก่อน) → P-18-followup-rbac + P-15-followup → P-19-followup batch+stale → P-21 templates → P-24 tests
4. **เดฟ:** D-12-wire + D-13-wire (~2-4h ทั้งคู่) — drop into signup/contact/password-reset
5. **ก๊อต:** schedule Pacred owner call:
   - D-7 payment gateway choice
   - LINE OA channel access token
   - Sentry account → DSN
   - Upstash Redis → URL+token
   - hCaptcha account → site/secret keys
