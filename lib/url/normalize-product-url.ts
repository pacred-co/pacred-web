 ...
 * + JD before storing in `tb_cart.curl` (varchar(300)).
 *
 ...
 *   4. Returns the original input as-is if URL parsing fails (fail-open · the
 *      caller's DB constraint will still catch >300 char + add it to the
 *      followup audit · but for non-URL strings like "ขอตามนี้นะ" we don't
 *      want to mangle them).
 */

// Postgres `tb_cart.curl varchar(300)` — leave 10-char buffer for safety
// (some marketplaces append `?` even on canonical URLs).
const MAX_URL_CHARS = 290;
 ...
 * @returns a normalized URL ≤ 290 chars, or a truncated fallback
 */
