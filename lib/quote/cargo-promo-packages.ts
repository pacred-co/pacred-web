...
/**
 * อย. · พิเศษ (FDA / น้ำยา / ควบคุม · รหัสสินค้า 3–4) — เรท **ค่าเริ่มต้น**
 * (ชั้นสุดท้ายของ waterfall) สินค้า อย./ควบคุม คิดแพงกว่าทั่วไป → เรทเดียว
 * **ทุกโกดัง** (ไม่บวก อี้อู +600) · **ทุกแพ็กเกจ**. เฉพาะ ฿/คิว + ฿/กก. —
 * "ระยะเวลา" ยังมาจากแพ็กเกจตามเดิม.
 *
 * ⚠️ owner 2026-07-21 (supersedes ปอน 2026-07-17/18): เดิมเป็น **เรทล็อกตายตัว
 * 7,600/6,600 ที่ override ทุกชั้น** (SVIP/แพ็ก/เรททั่วไป) และแก้ในใบเสนอราคาไม่ได้ →
 * ปลดล็อกแล้ว ให้แก้ได้เหมือน ทั่วไป·มอก. + ปรับค่าเริ่มต้นเป็น 7,500/6,500
 * (เรท ฿/กก. คงเดิม 45/35 ตามที่ owner สั่ง "เรทกิโลเท่าเดิม").
 * ตอนนี้เป็นแค่ fallback ชั้นสุดท้าย — ปกติจะอ่านเจอ tb_rate_g_* (7,500/6,500) ก่อน.
 */
export const FDA_SPECIAL_RATE: Record<QuoteMode, { cbm: number; kg: number }> = {
  truck: { cbm: 7500, kg: 45 },
  ship: { cbm: 6500, kg: 35 },
};
