...
The summary + the AGENTS.md top save-point + memory carry the thread. On the FIRST turn after a compaction: read the save-point + scroll the recent owner messages, reconstruct (a) the active task, (b) open owner asks, (c) decisions in force — THEN reply. Don't reply from a half-loaded thread.
