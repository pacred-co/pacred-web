 ...
 *
 * One-shot SMS gateway probe — auth-gated by CRON_SECRET so it can be
 * curled from any terminal. Built urgently 2026-05-20 night for the
 * "ส่ง SMS ไม่สำเร็จ" prod incident: register form was returning
 * `sms_failed` and we needed a way to see the real ThaiBulkSMS reply
 * without waiting on the next failed user to surface in Vercel logs.
 *
 *   curl -H "authorization: Bearer $CRON_SECRET" \
 *     "https://pacred.co.th/api/admin/sms-diagnostic?phone=0626030456"
 *
 * Response shape (always 200 — read the JSON):
 *   {
