...
/**
 * CustomerCoverUpload — the customer-facing "เปลี่ยนพื้นหลัง" button overlaid on
 * their /profile cover (Facebook-style). Mirrors the admin <CoverEditor> UX but
 * sets the CUSTOMER's OWN cover (actions/profile-cover-self.ts) — never the
 * global admin banner. §0f confirm-before-mutate: a slim dialog with a live
...
import { updateMyCover, resetMyCover } from "@/actions/profile-cover-self";

export function CustomerCoverUpload({ hasCustom }: { hasCustom: boolean }) {
  ...
  function submit(e: React.FormEvent) {
    ...
    fd.set("cover", file);
    start(async () => {
      const res = await updateMyCover(fd);
      if (!res.ok) {
        setError(res.error);
        return;
      }
        ...
        >
          ...
          >
            <div className="flex items-center justify-between bg-primary-600 px-4 py-3">
              <h3 className="text-sm font-semibold text-white">เปลี่ยนรูปพื้นหลัง</h3>
              <button
                type="button"
                onClick={close}
            ...
            <form onSubmit={submit} className="px-4 py-4">
              <p className="mb-3 text-xs leading-relaxed text-gray-500">
                แนะนำ{" "}
                <strong className="font-semibold text-gray-700 dark:text-foreground">1920 × 320 px</strong>{" "}
                · อัตราส่วน 6:1 (แนวนอน)
                <br />
                JPG / PNG / GIF / WEBP · ไม่เกิน 5 MB · เป็นปกของคุณเอง
              </p>
