...
export default async function ServiceImportReceiptsPage({
  ...
  const t = await getTranslations("serviceImportReceiptsPage");
            ...
            <form className="mt-3" method="GET" action="">
              ...
              {hasDateParam ? (
                <p className="mt-2 text-xs text-red-600">
                  {t("searchResult", { startDate, endDate })}{" "}
                </p>
              ) : null}
            </form>
          </div>
                    ...
                    >
                      <thead className="bg-surface-alt/50 text-left text-xs uppercase tracking-wide text-muted">
                        <tr>
                          ...
                          <th className="px-4 py-3 text-center font-medium">{t("printReceipt")}</th>
                        </tr>
                      </thead>
                      <tbody>
      ...
      {btn ? (
        <div className="fixed inset-x-0 z-[40] bottom-24 md:bottom-4 flex justify-center md:justify-end px-3 md:px-6 md:pr-[88px]">
          <button
            type="submit"
            id="select1"
            className="inline-flex items-center gap-2 rounded-full bg-emerald-600 text-white px-5 py-2.5 text-sm font-bold shadow-sm shadow-emerald-600/30 hover:bg-emerald-700 active:scale-[0.98] transition-all"
            name="type"
            value="1"
          >
