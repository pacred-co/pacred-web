...
import {
  adminUpsertGeneralRate,
  adminDeleteGeneralRate,
} from "@/actions/admin/rates";
import type { Row } from "./page";

// LP-1 — two-component module:
//   <GeneralRateRow>     — inline edit on an existing rate row
//   <NewGeneralRateRow>  — full-form add at the bottom
...
export function GeneralRateRow({
  row,
  warehouseLabel,
  transportLabel,
  productLabel,
  ...
  const [tier1, setTier1] = useState<string>(row.tier1?.toString() ?? "");
  const [tier2, setTier2] = useState<string>(row.tier2?.toString() ?? "");
  const [tier3, setTier3] = useState<string>(row.tier3?.toString() ?? "");
  const [err, setErr]     = useState<string | null>(null);
  const [msg, setMsg]     = useState<string | null>(null);

  const dirty =
    tier1 !== (row.tier1?.toString() ?? "") ||
    tier2 !== (row.tier2?.toString() ?? "") ||
    tier3 !== (row.tier3?.toString() ?? "");
  ...
  function save() {
    setErr(null);
    const t1 = parseTier(tier1);
    const t2 = parseTier(tier2);
    const t3 = parseTier(tier3);
    if (Number.isNaN(t1) || Number.isNaN(t2) || Number.isNaN(t3)) {
      setErr("tier ต้องเป็นตัวเลข ≥ 0 (หรือว่างไว้เพื่อ fall-through)");
      return;
    }
    startTransition(async () => {
      const res = await adminUpsertGeneralRate({
  ...
  function del() {
    if (!confirm(`ลบเรท ${row.source_warehouse}/${row.transport_type}/${row.product_type}/${row.basis} ?`)) return;
    startTransition(async () => {
      const res = await adminDeleteGeneralRate({ id: row.id });
    ...
    <>
      <tr className="border-t border-border">
        ...
        <td className="px-4 py-3 text-right">
          <input value={tier1} onChange={(e) => setTier1(e.target.value)} className={inputCls} placeholder="—" inputMode="decimal" disabled={pending} />
        </td>
        <td className="px-4 py-3 text-right">
          <input value={tier2} onChange={(e) => setTier2(e.target.value)} className={inputCls} placeholder="—" inputMode="decimal" disabled={pending} />
        </td>
        <td className="px-4 py-3 text-right">
          <input value={tier3} onChange={(e) => setTier3(e.target.value)} className={inputCls} placeholder="—" inputMode="decimal" disabled={pending} />
        </td>
...
export function NewGeneralRateRow({ defaultGroup }: { defaultGroup: string }) {
  const router = useRouter();
  const [pending, startTransition] = useTransition();
  ...
  function submit(e: React.FormEvent) {
    ...
    const t1 = parse(tier1), t2 = parse(tier2), t3 = parse(tier3);
    if (Number.isNaN(t1) || Number.isNaN(t2) || Number.isNaN(t3)) {
      setErr("tier ต้องเป็นตัวเลข ≥ 0 (หรือว่าง)");
      return;
    }
    if (t1 == null && t2 == null && t3 == null) {
      setErr("ต้องมีอย่างน้อย 1 tier");
      return;
    }
    startTransition(async () => {
      const res = await adminUpsertGeneralRate({
  ...
  return (
    <form onSubmit={submit} className="rounded-2xl border border-primary-200 bg-primary-50/30 p-4 shadow-sm space-y-3">
      <div className="flex items-center justify-between">
        <h3 className="font-bold text-sm">เรทใหม่ ({defaultGroup})</h3>
        <button type="button" onClick={() => { setOpen(false); reset(); }} className="text-xs text-muted hover:underline">ปิด</button>
      </div>
      <div className="grid sm:grid-cols-4 gap-2">
        <label className="block space-y-0.5">
          <span className="text-[10px] text-muted">โกดัง</span>
          <select value={warehouse} onChange={(e) => setWarehouse(e.target.value as typeof warehouse)} className={selectCls} disabled={pending}>
        ...
        <label className="block space-y-0.5">
          ...
          <select value={transport} onChange={(e) => setTransport(e.target.value as typeof transport)} className={selectCls} disabled={pending}>
        ...
        <label className="block space-y-0.5">
          ...
          <select value={product} onChange={(e) => setProduct(e.target.value as typeof product)} className={selectCls} disabled={pending}>
        ...
        <label className="block space-y-0.5">
          ...
          <select value={basis} onChange={(e) => setBasis(e.target.value as typeof basis)} className={selectCls} disabled={pending}>
      ...
      <div className="grid grid-cols-3 gap-2">
        <label className="block space-y-0.5">
          <span className="text-[10px] text-muted">tier1</span>
          <input value={tier1} onChange={(e) => setTier1(e.target.value)} className={inputCls + " w-full"} placeholder="—" inputMode="decimal" disabled={pending} />
        </label>
        <label className="block space-y-0.5">
          <span className="text-[10px] text-muted">tier2</span>
          <input value={tier2} onChange={(e) => setTier2(e.target.value)} className={inputCls + " w-full"} placeholder="—" inputMode="decimal" disabled={pending} />
        </label>
        <label className="block space-y-0.5">
          <span className="text-[10px] text-muted">tier3</span>
          <input value={tier3} onChange={(e) => setTier3(e.target.value)} className={inputCls + " w-full"} placeholder="—" inputMode="decimal" disabled={pending} />
        </label>
      </div>
      ...
      <p className="text-[10px] text-muted">
        ถ้าคีย์ตรงกับแถวที่มีอยู่ (กลุ่ม + โกดัง + ขนส่ง + ประเภท + หน่วย) จะถูกอัปเดตค่าทับ
      </p>
    </form>
  );
}
