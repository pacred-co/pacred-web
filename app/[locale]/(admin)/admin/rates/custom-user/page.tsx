/**
 * /admin/rates/custom-user — Custom rate overrides per customer (Phase A pending).
 *
 * Wave 7.2 (2026-05-21 night): the previous page queried + mutated the
 * rebuilt `rate_custom_user` table (empty on prod). The backing legacy
 * tables (`tb_priceuser_member`, `tb_priceuser_hs`) have NOT been
 * migrated to Supabase yet — only the base rate defaults in
 * `tb_settings` (rgdefault, rsdefault, rpdefault) are on prod.
 *
 ...
 * other surface reads → would silently break custom pricing for VIP /
    ...
    <main className="p-6 lg:p-8 space-y-5 max-w-3xl">
      ...
      <div className="rounded-2xl border border-yellow-200 bg-yellow-50 p-5 space-y-3 text-sm">
        <p className="font-medium text-yellow-900">
          ตาราง <code className="rounded bg-yellow-100 px-1.5 py-0.5">tb_priceuser_member</code>{" "}
          ยังไม่ถูกย้ายจาก legacy MySQL → Supabase (Phase A migration backlog)
        </p>
        <p className="text-yellow-800">
          ข้อมูล custom rate ต่อลูกค้า (VIP / customer-specific overrides) ยังอยู่ที่
          legacy DB ตอนนี้ การคำนวณราคาขนส่ง ปัจจุบันใช้ default rates จาก{" "}
          <code className="rounded bg-yellow-100 px-1.5 py-0.5">tb_settings</code>
          {" "}(rgdefault / rsdefault / rpdefault) เท่านั้น · custom overrides จะกลับมาทำงาน
          เมื่อ migration Phase A เสร็จ
        </p>
        <p className="text-yellow-800 font-medium">วิธีดู / แก้ไข custom rate ชั่วคราว:</p>
        <ol className="list-decimal pl-6 text-yellow-800 space-y-1">
          <li>
            ...
            <code className="rounded bg-yellow-100 px-1.5 py-0.5">
              pcs-admin/rate-custom-user.php
            </code>
            ) สำหรับเพิ่ม / แก้ไข custom rate ตามลูกค้า
          </li>
          <li>
