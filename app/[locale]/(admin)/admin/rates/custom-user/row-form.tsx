...
import {
  adminUpsertCustomUserRate,
  adminDeleteCustomUserRate,
...
export function CustomUserRateRow({
  row,
  memberCell,
  warehouseLabel,
  transportLabel,
  ...
  row:            FlatRow;
  memberCell:     { code: string | null; name: string } | null;  // null = repeat customer row (merge cells)
  ...
  function save() {
    ...
    startTransition(async () => {
      const res = await adminUpsertCustomUserRate({
        customer_ref:     row.profile_id,   // already-resolved UUID
  ...
  function del() {
    if (!confirm(`ลบ custom rate ${row.source_warehouse}/${row.transport_type}/${row.product_type}/${row.basis} ของลูกค้านี้ ?`)) return;
    startTransition(async () => {
      const res = await adminDeleteCustomUserRate({ id: row.id });
    ...
    <>
      <tr className="border-t border-border">
        <td className="px-4 py-3 text-xs">
          {memberCell ? (
            <>
              <p className="font-medium">{memberCell.name}</p>
              {memberCell.code && <p className="font-mono text-[10px] text-muted">{memberCell.code}</p>}
            </>
          ) : (
...
export function NewCustomUserRateRow({ defaultMember }: { defaultMember: string }) {
  const router = useRouter();
  const [pending, startTransition] = useTransition();
  ...
  function submit(e: React.FormEvent) {
    ...
    startTransition(async () => {
      ...
      if (res.ok && res.data) {
        setMsg(`✓ บันทึกให้ ${res.data.member_code ?? res.data.profile_id.slice(0, 8)}`);
        setRate("");
        // keep customerRef for next add to same person
        router.refresh();
        setTimeout(() => setMsg(null), 4000);
      } else if (!res.ok) {
        setErr(res.error);
  ...
  return (
    <form onSubmit={submit} className="rounded-2xl border border-primary-200 bg-primary-50/30 p-4 shadow-sm space-y-3">
      <div className="flex items-center justify-between">
        <h3 className="font-bold text-sm">Custom rate ใหม่</h3>
        <button type="button" onClick={() => { setOpen(false); setRate(""); setErr(null); setMsg(null); }} className="text-xs text-muted hover:underline">ปิด</button>
      </div>
      ...
      <p className="text-[10px] text-muted">
        ถ้าคีย์ตรงกับที่มีอยู่ (ลูกค้า + โกดัง + ขนส่ง + ประเภท + หน่วย) จะถูกอัปเดต rate ทับ
      </p>
    </form>
  );
}
