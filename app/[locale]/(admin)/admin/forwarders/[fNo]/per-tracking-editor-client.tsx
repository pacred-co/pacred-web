...
export function PerTrackingEditorClient({
  ...
  // ── ✎ IDENTITY inline-edit (owner 2026-07-21 → ภูม 2026-07-21 "เอาแบบดินสอ inline
  // เหมือนหน้า momo-containers · ไม่ต้องกดปุ่มปลดล็อกทั้งตาราง มันยุ่งยากไป") ──
  // Each identity field (เลขแทรคกิ้ง · เลขชิปเม้น · จำนวนกล่อง) shows its value with a
  // small ✎ pencil; click → that ONE field turns into an inline input. Billed rows
  // (fstatus ≥ 6) stay read-only (accounting owns them). The save-gate `unlocked` is
  // DERIVED from "any identity field is open for edit" → the persist logic below
  // (renames / boxChanges) is byte-identical: nothing identity-related is written
  // unless a pencil was opened.
  const [editTracking, setEditTracking] = useState<Set<number>>(new Set());
  const [editBoxes, setEditBoxes] = useState<Set<number>>(new Set());
  const [editBase, setEditBase] = useState(false);
  const unlocked = editBase || editTracking.size > 0 || editBoxes.size > 0;
  ...
  return (
    <div className="space-y-3">
      ...
      {/* ── ✎ เลขชิปเม้น (ฐาน) inline-edit + ↺ reset (owner 2026-07-21 · ภูม: pencil
          inline แบบ momo-containers · ไม่ต้องปลดล็อกทั้งตาราง) ── */}
      <div className="flex flex-wrap items-center gap-2 rounded-lg border border-border bg-surface-alt/30 px-3 py-2">
        <div className="flex items-center gap-1.5 text-xs">
          ...
          {editBase ? (
            <>
              ...
              />
              <span className="text-[11px] text-muted">(เปลี่ยนแล้วเลขแทรคกิ้งทุกกล่องเปลี่ยนตาม -N/M เดิม · แล้วกด “บันทึก”)</span>
              <button type="button" disabled={pending} title="ยกเลิกการแก้ไข"
                onClick={() => { setEditBase(false); setBaseInput(initialBase); setRows((rs) => rs.map((r, i) => rowIsBilled(r) ? r : ({ ...r, tracking: origTrackings[i] ?? r.tracking }))); }}
                className="rounded border border-border px-1.5 py-0.5 text-[11px] text-muted hover:bg-surface-alt">✕</button>
            </>
          ) : (
            <>
              <span className="font-mono font-medium">{baseInput || "—"}</span>
              <button type="button" disabled={pending} title="แก้ไขเลขชิปเม้น (ฐาน)"
                onClick={() => setEditBase(true)}
                className="rounded border border-border bg-white dark:bg-surface px-1.5 py-0.5 text-[12px] leading-none text-slate-500 hover:bg-surface-alt hover:text-primary-600">✎</button>
            </>
          )}
        </div>
            ...
            {rows.map((r, idx) => (
              <tr key={r.id} className="border-t border-border align-top [&>td]:px-1.5 [&>td]:py-1.5 [&>td]:border-r [&>td]:border-border">
                <td className="min-w-[170px] max-w-[260px] text-left">
                  {rowIsBilled(r) ? (
                    <div className="font-mono text-[11px] font-medium break-words">{r.tracking || "—"}<span className="ml-1 text-[10px] text-muted">🔒 บิลแล้ว</span></div>
                  ) : editTracking.has(idx) ? (
                    <div className="flex items-center gap-1">
                      ...
                      />
                      <button type="button" disabled={pending} title="ยกเลิก"
                        onClick={() => { patch(idx, { tracking: origTrackings[idx] ?? r.tracking }); setEditTracking((s) => { const n = new Set(s); n.delete(idx); return n; }); }}
                        className="shrink-0 rounded border border-border px-1 py-0.5 text-[11px] text-muted hover:bg-surface-alt">✕</button>
                    </div>
                  ) : (
                    <div className="flex items-center gap-1">
                      <span className="font-mono text-[11px] font-medium break-words">{r.tracking || "—"}</span>
                      <button type="button" disabled={pending} title="แก้ไขเลขแทรคกิ้ง"
                        onClick={() => setEditTracking((s) => new Set(s).add(idx))}
                        className="shrink-0 rounded border border-border bg-white dark:bg-surface px-1 py-0.5 text-[11px] leading-none text-slate-400 hover:bg-surface-alt hover:text-primary-600">✎</button>
                    </div>
                  )}
                ...
                <td className="text-center font-mono text-xs text-muted">
                  {rowIsBilled(r) ? (
                    ...
                    <div className="flex items-center justify-center gap-1">
                      ...
                      />
                      <button type="button" disabled={pending} title="ยกเลิก"
                        onClick={() => { patch(idx, { boxes: String(origBoxes[idx] ?? r.boxes) }); setEditBoxes((s) => { const n = new Set(s); n.delete(idx); return n; }); }}
                        className="shrink-0 rounded border border-border px-1 py-0.5 text-[11px] text-muted hover:bg-surface-alt">✕</button>
                    </div>
                  ) : (
                    <span className="inline-flex items-center justify-center gap-1">
                      <span>{r.boxes}</span>
                      <button type="button" disabled={pending} title="แก้ไขจำนวนกล่อง"
                        onClick={() => setEditBoxes((s) => new Set(s).add(idx))}
                        className="shrink-0 rounded border border-border bg-white dark:bg-surface px-1 py-0.5 text-[11px] leading-none text-slate-400 hover:bg-surface-alt hover:text-primary-600">✎</button>
                    </span>
                  )}
                </td>
