/**
 * /admin/api-sheets-ctt — manual forwarder entry through the CTT warehouse.
 *
 * Wave 17 P1-3 — faithful port of legacy `pcs-admin/api-sheets-ctt.php` (1352 LOC).
 * Despite the file name, the legacy page is NOT a Google-Sheets API consumer —
 * it's a manual entry form that always sets `fWarehouseName=2` (CTT) on INSERT.
 *
 * One shared client form (<CarrierManualForm>) renders for all 4 carriers
 * (CTT · Sang · MK · MX). The carrier registry (lib/carrier/registry.ts)
 * supplies the warehouse code + label + decorative Sheets link.
 */
...
export default async function ApiSheetsCttPage({
  searchParams,
}: {
  ...
  const carrier = CARRIER_REGISTRY.ctt;
