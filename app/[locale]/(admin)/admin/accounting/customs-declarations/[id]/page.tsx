 ...
 *
 * The EDIT wrapper for the accounting ใบขนสินค้า hub. The list page
 * (../page.tsx) was read-only (rows only linked to the PDF) while the full
 * mutate workflow already existed: `actions/admin/customs-declarations.ts`
 * (create/update header/lines/submit/accept/release/cancel) + the editable
 * client island `<DeclarationDetailClient>` (built for /admin/freight). This
 * page mounts that exact client under the accounting route so accounting
 * staff get the same draft-edit + status workflow without a duplicate UI.
 *
 * Identical auth + fetch to /admin/freight/declarations/[id]/page.tsx — only
 * the chrome (CARGO_MENUBAR + breadcrumb back to the accounting list) differs.
 * Roles per ADR-0014 + the ops-workflow unlock: super | accounting | Doc roles.
 *
 * MONEY/TAX SAFETY: all mutations stay inside the existing customs actions
 * (draft-only header/line edits + the documented status lifecycle). No new
 * money path; declared values are the audited manual field, never auto-derived.
 */
...
export default async function AdminAccountingCustomsDeclarationDetailPage({
  params,
}: {
  ...
  const { data: header, error: headerErr } = await admin
    ...
    .maybeSingle<Header>();
  if (headerErr) {
    console.error(`[acc customs_declarations lookup] failed`, { code: headerErr.code, message: headerErr.message, details: headerErr.details, hint: headerErr.hint });
  ...
  if (shipmentErr) {
    console.error(`[acc freight_shipments lookup] failed`, { code: shipmentErr.code, message: shipmentErr.message });
  }
  ...
  if (customerErr) {
    console.error(`[acc profiles lookup] failed`, { code: customerErr.code, message: customerErr.message });
  }
  ...
  if (linesRawErr) {
    console.error(`[acc customs_declaration_lines list] failed`, { code: linesRawErr.code, message: linesRawErr.message });
  }
    ...
    <>
      ...
      <main className="p-6 lg:p-8 space-y-5 max-w-6xl">
        <div className="flex items-center justify-between flex-wrap gap-3">
          <div>
            <Link href="/admin/accounting/customs-declarations" className="text-xs text-primary-500 hover:underline">
              ← กลับหน้ารายการใบขน
            </Link>
        ...
        {/* Editable header + line CRUD + status actions (shared client island) */}
