  ...
  return (
    <main className="p-6 lg:p-8 space-y-6 max-w-6xl">
      ...
      <div className="flex flex-wrap gap-2">
        {FILTER_TABS.map((t) => {
          ...
          return (
            ...
            >
              {t.label}
              <span className="ml-1.5 rounded-full bg-black/5 dark:bg-white/10 px-1.5 py-0.5 text-[11px] tabular-nums">
                {n}
              </span>
            </Link>
          );
      ...
      <section className="space-y-2">
                  ...
                  {rows.map((q) => (
                    <tr key={q.id} className="border-t border-border hover:bg-surface-alt/30">
                      ...
                      <td className="px-3 py-2">
                        <span className={`rounded-full border px-2 py-0.5 text-[11px] whitespace-nowrap ${STATUS_CLS[q.status]}`}>
        ...
        <p className="text-[11px] text-muted">⇆ เลื่อนซ้าย-ขวาเพื่อดูคอลัมน์ทั้งหมด</p>
      </section>
