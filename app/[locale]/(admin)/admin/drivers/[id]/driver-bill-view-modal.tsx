...
/**
 * ฟอร์ม "บิลใบส่งสินค้า" — มุมมองรวมพร้อมพิมพ์ ของทุกจุดส่งในรอบ จัดกลุ่มตาม
 * ลูกค้า/ที่อยู่ (ภูม 2026-07-10 · faithful port legacy forwarder-driver.php
 * `#listBill` → addFromBill.php action=3).
 *
 * โครง (ปอน 2026-07-23 rebuilt to legacy SHAPE):
 *   OUTER table  # | จำนวนรวม | บริษัทขนส่ง | ข้อมูล
 *     └─ ข้อมูล = INNER table  # | เลขออเดอร์ | รหัสสมาชิก | เลขแทรคกิ้ง |
 *        location | ที่อยู่ (พื้นชมพู · เบอร์ inline) + ปุ่ม "พิมพ์และบันทึกบิลรวม" ต่อกลุ่ม
 *   + list chrome: แสดง N รายการ · ค้นหา · zebra · "กำลังแสดง X ถึง Y จาก Z" + prev/next.
 *
 * 🔀 ปอน 2026-07-24 — แยก body ออกเป็น <DeliveryBillForm> ที่ใช้ซ้ำได้ 2 ที่:
 *   • วาง "บนหน้า" รายละเอียดรอบ (/admin/drivers/[id]) โดยตรง — owner "อยากได้ฟอร์มนั้น"
 *   • ใน popup <DriverBillViewModal> (ปุ่ม "ดูบิลใบส่งสินค้า" เปิดฟอร์มเดียวกัน)
 * → ฟอร์มเดียว ไม่ drift. Design = Pacred Tailwind. อ่านจาก stop data ที่หน้าโหลดมาแล้ว
 * (ไม่ fetch เพิ่ม · ไม่มี write).
 */
...
/**
 * DeliveryBillForm — the reusable bill form BODY (search + grouped table with
 * the per-group "พิมพ์และบันทึกบิลรวม" delivery-slip action + pagination).
 *
 * Rendered inline on /admin/drivers/[id] (ปอน 2026-07-24) AND inside
 * <DriverBillViewModal>. No max-height/scroll of its own — the caller supplies
 * the frame (the modal wraps it in a scroll box; the page wraps it in a card).
 */
export function DeliveryBillForm({
  groups,
  slipHref,
}: {
  groups: BillGroup[];
  /**
   * ใบส่งสินค้า base path. Each group appends its own `?fids=` so the slip
   * covers exactly that address's parcels.
   */
  slipHref: string;
}) {
                  ...
                  <td className="px-2 py-3">
                    ...
                    <div className="mt-2 flex flex-wrap items-center gap-2">
                      ...
                      <span className="text-[11px] text-muted">
                        พิมพ์ใบส่งสินค้าเฉพาะกลุ่มที่อยู่นี้
                      </span>
                    </div>
                  </td>
                </tr>
...
/**
 * DriverBillViewModal — the trigger button + a portal modal wrapping the SAME
 * <DeliveryBillForm>. (The batch page also renders <DeliveryBillForm> inline,
 * so this modal is the "quick-look" version of the on-page form.)
 */
export function DriverBillViewModal({
  ...
  /** ข้อความบนปุ่ม trigger — มือถือใช้ "ดูใบส่งของ" (ตามภาพ) · default "ดูบิลใบส่งสินค้า". */
  label?: string;
}) {
  ...
  return (
    <>
      ...
      {/* PORTAL TO BODY — the admin shell's `.animate-fade-in` wrapper opens a
          stacking/transform context; portaling to <body> lets z-[90] beat the
          fixed header (z-[60]) instead of only its siblings. */}
      {open && createPortal(
        <div className="fixed inset-0 z-[90] flex items-start justify-center overflow-y-auto overscroll-contain bg-black/50 p-3 sm:p-4">
          <div className="my-4 w-full max-w-6xl rounded-2xl bg-white shadow-xl sm:my-6">
            ...
            <div className="max-h-[72vh] overflow-y-auto p-3 sm:max-h-[78vh] sm:p-4">
              <DeliveryBillForm groups={groups} slipHref={slipHref} />
            </div>
          </div>
        </div>,
        document.body,
