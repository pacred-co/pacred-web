/**
 * <DriverRunDocument> — เอกสาร "รอบจัดส่ง" ตัวเดียว ใช้ร่วมกัน 2 หน้า:
 *   • /admin/drivers/[id]/print         → บิลจัดส่ง (คนขับ)
 *   • /admin/drivers/[id]/picking-list  → บิลหาสินค้า (คลัง)
 *
 * 🔴 owner 2026-07-23: "ฟอร์มเหมือนกันแบบ 1:1 ยึดบิลจัดส่งเป็นหลัก … เหมือนกันเลย
 * แค่หัวต่างกันเฉยๆ" — owner รับทราบแล้วว่าบิลหาสินค้าจะเสียช่องติ๊กหยิบ · รูปสินค้า ·
 * การจัดกลุ่มตามตำแหน่งชั้นวาง และยืนยันให้ทำ.
 *
 * ทำเป็น "คอมโพเนนต์เดียว + variant" แทนการ copy โค้ดไปสองที่ เพราะถ้า copy แล้ว
 * วันหลังมีคนแก้ฝั่งเดียว มันจะกลับมาต่างกันอีก (ซึ่งคือปัญหาที่เพิ่งแก้อยู่นี้).
 * ต่างกันได้เฉพาะสิ่งที่อยู่ใน `variant` เท่านั้น — โครง/สี/ตาราง บังคับให้เหมือนกัน
 * เชิงโครงสร้าง.
 */
...
/** สิ่งเดียวที่สองใบต่างกันได้ — นอกเหนือจากนี้บังคับเหมือนกัน. */
export type RunDocVariant = {
  /** ชื่อเอกสารตัวใหญ่มุมขวาบน */
  title: string;
  /** บรรทัดอังกฤษใต้ชื่อ */
  subtitle: string;
  /** ป้ายบนปุ่มพิมพ์ */
  printLabel: string;
  /** ลิงก์ข้ามไปเอกสารอีกใบ (บนแถบเครื่องมือ · ไม่พิมพ์) */
  crossLinkHref: (batchId: number) => string;
...
export const RUN_DOC_DELIVERY: RunDocVariant = {
  title: "บิลจัดส่ง",
  subtitle: "Delivery Note (คนขับ)",
  printLabel: "🖨 พิมพ์บิลจัดส่ง",
...
export const RUN_DOC_PICKING: RunDocVariant = {
  title: "บิลหาสินค้า",
  subtitle: "Picking List (คลัง)",
  printLabel: "🖨 พิมพ์บิลหาสินค้า",
...
export async function DriverRunDocument({
  params,
  variant,
}: {
  params: Promise<{ id: string }>;
  variant: RunDocVariant;
}) {
  ...
  // (แถบชิปเลขใบวางบิล/ใบเสร็จบนแถบเครื่องมือถูกเอาออก 2026-07-23 ตาม owner
  //  "เอาเลขบิลออก มันดูยากจัด" — พร้อมกับ 4 query ที่ดึงมาโชว์ ซึ่งตอนนี้ไม่มีใครใช้
  //  แล้ว จึงตัดทิ้งด้วย = หน้าโหลดเร็วขึ้นอีก 4 query ต่อครั้ง. เอกสารอ้างอิงยังเข้าถึง
  //  ได้จากหน้ารายละเอียดรอบตามปกติ.)
    ...
    <div className="doc-desk min-h-screen bg-slate-100 text-slate-900">
      ...
      <div className="no-print sticky top-0 z-10 flex flex-wrap items-center justify-between gap-2 border-b border-gray-200 bg-white/90 px-4 py-3 backdrop-blur">
        <div className="flex items-center gap-3 text-sm">
          ...
          <Link
            href={variant.crossLinkHref(batch.id)}
            target="_blank"
            className="text-primary-600 hover:underline"
          >
            {variant.crossLinkLabel}
          </Link>
          <span className="text-xs text-gray-500">
            {variant.shortName} · รอบ #{batch.id} · {totalTrackings} แทรคกิ้ง
          </span>
        </div>
        <PrintButton label={variant.printLabel} />
      </div>
      ...
      <main className="print-area mx-auto my-6 max-w-[860px] overflow-hidden rounded-xl border border-slate-200 bg-white shadow-[0_1px_3px_rgba(0,0,0,0.08),0_6px_20px_rgba(0,0,0,0.06)]">
        ...
        <div className="flex items-start justify-between gap-6">
          ...
          <div className="w-[46%] max-w-[320px] shrink-0">
            <DocTitle title={variant.title} subtitle={variant.subtitle} />
            <DocMetaBox>
