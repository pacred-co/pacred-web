...
/**
 * Client buttons + forms for the admin-profile page. The legacy
 * (admin-profile.php) uses jQuery + Bootstrap-4 modals + SweetAlert
 * toasts; per faithful-port runbook §8 + §9 gotcha #3 the Bootstrap-4
 * data-toggle / data-target / .modal markup is transcribed VERBATIM
 * (vendor JS loads from the (protected) layout — jQuery + BS4 run at
 * runtime). What we replace:
 *   - Server Action calls (instead of jQuery $.ajax to .php endpoints)
 *   - Plain inline "สำเร็จ / ผิดพลาด" status text (instead of SweetAlert)
 *
 * Each button below maps 1:1 to the legacy `data-target="#…"` modal.
 *
 * IMPORTANT — this is one file with one default export (an object
 * grouping every named subcomponent). That keeps the parent page
 * import simple (`import { AdminProfileClient }`) while preserving
 * per-button isolation.
 */
...
function SetCommCog({ adminId, currentPerCom }: { adminId: string; currentPerCom: number }) {
  ...
  return (
    <>
      <a href="#" data-toggle="modal" data-target="#set-comm-interpreter">
        <span className="d-inline-block badge-warning badge-pill pl-1 pr-1 text-white">
          <button type="button" className="btn tn-icon text-white btn-pure p-0 pull-up">
      ...
      <div id="set-comm-interpreter" className="modal fade in" tabIndex={-1} role="dialog" aria-hidden="true">
        <div className="modal-dialog">
          <div className="modal-content header-from">
            <div className="modal-header">
              <h4 className="modal-title">ตั้งค่าค่าคอมล่ามจีน {adminId}</h4>
              <button type="button" className="close" data-dismiss="modal" aria-hidden="true"><i className="la la-close"> </i></button>
            </div>
            <div className="modal-body header-from">
              <form className="form-horizontal" onSubmit={onSubmit} autoComplete="off">
                <div className="mb-1">
                  <label className="form-control-label" htmlFor="perCom">กรอกเปอร์เซ็นของค่าคอมที่ต้องการให้ ยึดจากผลต่างที่ต่อรองราคาได้</label>
                  <input id="perCom" value={perCom} onChange={(e) => setPerCom(e.target.value)} className="form-control form-control-lg" placeholder="เช่น 5%" type="number" max={100} step={0.01} pattern="\d*" required />
                </div>
                <StatusLine status={status} />
                <div className="modal-footer">
                  <button type="reset" className="btn btn-outline-secondary round waves-effect" data-dismiss="modal">ยกเลิก</button>
                  <button type="submit" className="btn btn-color-main round waves-effect" disabled={pending}>{pending ? "กำลังบันทึก..." : "บันทึก"}</button>
                </div>
              </form>
            </div>
          </div>
...
function SetFurloughCog({ adminId }: { adminId: string }) {
  ...
  return (
    <>
      <a href="#" data-toggle="modal" data-target="#set-furlough">
        <span className="d-inline-block badge-warning badge-pill pl-1 pr-1 text-white">
          <button type="button" className="btn tn-icon text-white btn-pure p-0 pull-up">
            <svg viewBox="0 0 24 24" width={24} height={24} stroke="currentColor" strokeWidth={2} fill="none" strokeLinecap="round" strokeLinejoin="round">
              <circle cx="12" cy="12" r="3" />
      ...
      <div id="set-furlough" className="modal fade in" tabIndex={-1} role="dialog" aria-hidden="true">
        <div className="modal-dialog">
          <div className="modal-content header-from">
            <div className="modal-header">
              <h4 className="modal-title">ตั้งค่าพักงานชั่วคราว/ปิดรับออเดอร์ {adminId}</h4>
              <button type="button" className="close" data-dismiss="modal" aria-hidden="true"><i className="la la-close"> </i></button>
            </div>
            <div className="modal-body header-from">
              <form className="form-horizontal" onSubmit={onSubmit} autoComplete="off">
                <div className="mb-1">
                  ...
                  <select name="adminTMP" value={adminTmp} onChange={(e) => setAdminTmp(e.target.value)}>
...
function EditProfileButton({
  ...
  return (
    <>
      <a href="#" data-toggle="modal" data-target="#form-updateProfile">
        <span className="d-inline-block badge-warning badge-pill pl-1 pr-1 text-white">
          <button type="button" className="btn tn-icon text-white btn-pure p-0 pull-up">
      ...
      <div id="form-updateProfile" className="modal fade in" tabIndex={-1} role="dialog" aria-hidden="true">
        <div className="modal-dialog modal-lg">
          <div className="modal-content header-from">
            <div className="modal-header">
              <h4 className="modal-title">แก้ไขข้อมูล {adminId}</h4>
              <button type="button" className="close" data-dismiss="modal" aria-hidden="true"><i className="la la-close"> </i></button>
            </div>
            <div className="modal-body header-from text-left">
              <form className="form-horizontal" onSubmit={onSubmit} autoComplete="off">
                {showJobPosition && (
                ...
                <div className="form-posit">
                  ...
                  <div className="row mb-1">
                    <div className="col-md-6">
                      ...
                      <select value={v.admin_email_org} onChange={up("admin_email_org")} style={{ width: "100%" }} className="select3 form-control form-control-lg">
                        <option value="">กรุณาเลือก</option>
                    ...
                    <div className="col-md-6">
                      ...
                      <select value={v.admin_tel_org} onChange={up("admin_tel_org")} style={{ width: "100%" }} className="select3 form-control form-control-lg">
                        <option value="">กรุณาเลือก</option>
                  ...
                  <div className="row mb-1">
                    <div className="col-md-6">
                      ...
                      <select value={v.admin_line_org} onChange={up("admin_line_org")} style={{ width: "100%" }} className="select3 form-control form-control-lg">
                        <option value="">กรุณาเลือก</option>
                    ...
                    <div className="col-md-6">
                      ...
                      <select value={v.admin_wechat_org} onChange={up("admin_wechat_org")} style={{ width: "100%" }} className="select3 form-control form-control-lg">
                        <option value="">กรุณาเลือก</option>
                ...
                <div className="modal-footer">
                  <button type="reset" className="btn btn-outline-secondary round waves-effect" data-dismiss="modal">ยกเลิก</button>
                  <button type="submit" className="btn btn-color-main round waves-effect" disabled={pending}>{pending ? "กำลังบันทึก..." : "อัปเดตข้อมูล"}</button>
                </div>
              </form>
            </div>
          </div>
...
function AddBankAccountButton({ adminId }: { adminId: string }) {
  ...
  return (
    <>
      <a href="#" data-toggle="modal" data-target="#add-accAdmin">
        <button type="button" className="btn btn-sm btn-circle btn-success text-white">
          <i className="ft-plus"></i>
        </button>
        <span className="font-normal text-dark">เพิ่มข้อมูล</span>
      </a>
      <div id="add-accAdmin" className="modal fade in" tabIndex={-1} role="dialog" aria-hidden="true">
        <div className="modal-dialog">
          <div className="modal-content header-from">
            <div className="modal-header">
              <h4 className="modal-title">เพิ่มเลขที่บัญชีธนาคาร ให้กับ {adminId}</h4>
              <button type="button" className="close" data-dismiss="modal" aria-hidden="true"><i className="la la-close"> </i></button>
            </div>
            <div className="modal-body header-from">
              <form className="form-horizontal" onSubmit={onSubmit} autoComplete="off">
                <div className="mb-1">
                  <select name="bankName" value={bankName} onChange={(e) => setBankName(e.target.value)} className="select3 form-control form-control-lg" style={{ width: "100%" }} required>
...
function DeleteBankButton({ accountId, adminId }: { accountId: number; adminId: string }) {
  ...
  const [pending, startTransition] = useTransition();
  function onClick() {
    if (!confirm("ต้องการลบเลขที่บัญชีนี้?")) return;
    startTransition(async () => {
  ...
  return (
    <a href="#" onClick={(e) => { e.preventDefault(); onClick(); }}>
      <p className="btn font-12 btn-sm btn-danger btn-rounded">{pending ? "กำลังลบ..." : "ลบข้อมูล"}</p>
    </a>
  );
}
...
function AddEducationButton({ adminId }: { adminId: string }) {
  ...
  return (
    <>
      <a href="#" data-toggle="modal" data-target="#add-educationAdmin">
        <button type="button" className="btn btn-sm btn-circle btn-success text-white">
          <i className="ft-plus"></i>
        </button>
        <span className="font-normal text-dark">เพิ่มข้อมูล</span>
      </a>
      <div id="add-educationAdmin" className="modal fade in" tabIndex={-1} role="dialog" aria-hidden="true">
        <div className="modal-dialog modal-lg">
          <div className="modal-content header-from">
            <div className="modal-header">
              <h4 className="modal-title">เพิ่มประวัติการศึกษาให้กับ {adminId}</h4>
              <button type="button" className="close" data-dismiss="modal" aria-hidden="true"><i className="la la-close"> </i></button>
            </div>
            <div className="modal-body header-from">
              <form className="form-horizontal" onSubmit={onSubmit} autoComplete="off">
...
function DeleteEducationButton({ educationId, adminId }: { educationId: number; adminId: string }) {
  ...
  const [pending, startTransition] = useTransition();
  function onClick() {
    if (!confirm("ต้องการลบประวัติการศึกษานี้?")) return;
    startTransition(async () => {
