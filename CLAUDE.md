...
2. **ก่อนเริ่มงานทุกครั้ง** — sync `main` เข้า branch ตัวเองก่อน เพื่อรับแพช/update ล่าสุดที่ก๊อตเพิ่ง merge:
   ```bash
   git checkout <my-branch>
   git status                    # ต้องสะอาด — ถ้ามีไฟล์ค้างให้ commit หรือ stash ก่อน
   git fetch origin
   git pull origin main          # ดึง main เข้า branch ตัวเอง (รวมแพชใหม่ของก๊อต)
