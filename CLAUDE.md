...
> **State: dave-pacred = `4847222d`** (W1 `d6f466b4` money-safety · W2a `88214472` money-correctness · W3.1 `9c2c0bf7` notPortage combine-shipping · W3.2 `4847222d` shop print-flag). gate every push: typecheck 0 · lint 0 err · test:unit 0 fail. main NOT advanced (owner go only). **NEXT FREE migration = 0183.** No prod writes yet (W5 migration owner-APPROVED — run prod dup-precheck first). ⚙️ shell node-revert workaround unchanged (`export PATH=…v24.16.0/bin` + `corepack pnpm` + run typecheck/lint/test:unit individually).
>
> **WAVE PROGRESS (owner "ต่อยาวๆ"):** ✅ W1 (money-safety TOCTOU+dead-read+nav) · ✅ W2a (yuan cost capture + badges + zip-lib · WHT-col false-positive) · ✅ W3.1 (notPortage combine-shipping writer+UI+filter) · ✅ W3.2 (shop print-flag). **NEXT:** W3.3 manual MOMO/CN dedup+auto-price + manualUpdate edit · W3.4 per-fStatus tab strip · **W4 cnt-hs cost-correction (P0 · pure in-code)** · **W5 migration 0183 (P0 · owner-approved · prod dup-precheck FIRST · +tb_forwarder_tran_th_sub.fid added to scope)** · W6 JMF/TTP read-only + GOGO retire-banner · W7 polish. Per-wave status in `docs/research/forwarder-fidelity-audit-2026-06-14.md`.
>
