...
# 🧾 2026-06-12 — เดฟ: cargo-acct 10-gap plan **10/10 SHIPPED** + GAP 1 re-seed (real cost) + Build A (declared Customs-FX) + Build B (คลัง HS) + GAP 5 (CS HS-triage) + mig 0179/0180 prod **+ DEV=PROD reconcile** → dave-pacred + MAIN · read FIRST
...
>
> - **✅ GAP 5 CS HS-triage queue** (`b67d4447` + harden `ca692439`) — owner chose a **dedicated CS คิวงานรวม**: `/admin/accounting/hs-triage` lists every per-line item (`tb_forwarder_item`+`tb_order`) with no `hs_code` yet; CS types the HS with a **live คลัง HS duty hint** (`lookupHsCode` · reference-only), confirms, saves → the row drops out. `setLineHsCode` writes **ONLY `hs_code`** (§0e) · CS-gated `super/sales/sales_admin/ops`. The ground-truth flow (CS asks China warehouse → HS → THEN Pricing costs); the cost editor + cargo ใบขน already read `tb_*.hs_code` so it threads through.
>
...
>
> **🟦 NEXT-WAVE DIRECTION (needs an OWNER pick — no clear autonomous default):** the buildable-now candidates are **(A) config-SOT audit + ADR** (map yuan_rate/free_shipping reader/writer split-brain → pick the canonical store → ADR-0028 · audit-first, defer any risky repoint) · **(B) §0e dead-write-trap sweep** (re-verify admin rate/form writers vs their live consumers from source · produce a verified fix-list — the prior `rate-cost-wiring-audit-2026-06-11` did the big pass) · **(C) accounting-hub nav consolidation** (deeply-nested menubar pages → discoverable hub tabs · pure nav) · **(D) partner-API carrier adapters GOGO/JMF/TTP** (L · needs each carrier's creds/endpoints confirmed). Or **activate a dormant lever** (flip `shop_yuan_enabled` / `commission.freight_enabled` / set Customs FX + HS + GL data) — those are owner/accountant data-entry, code is DONE.
>
