...
> **State: dave-pacred = `d6f466b4`** (+Wave-1 money-safety · was `6e8efb41` disbursement-harden · was `918c421a`). gate every push: typecheck 0 · lint 0 err · test:unit 0 fail. main NOT advanced (owner go only). **NEXT FREE migration = 0183.** No prod writes this session. ⚙️ shell node-revert workaround unchanged (`export PATH=…v24.16.0/bin` + `corepack pnpm` + run typecheck/lint/test:unit individually).
>
...
>
> **🔴 OWNER FLAGS (from the audit · see research doc §Owner flags):** (1) **Wave-5 migration 0183** — approve a prod dup-precheck + merge/keep policy BEFORE the UNIQUE on tb_cnt_item.fcabinetnumber / tb_user_sales.idf / tb_user_sales_pay.idus (carryover). (2) **GOGO+JMF creds** (carryover · JMF view+history buildable read-only now). (3) **Yuan-approve cost source** — confirm payRateCost defaults from `tb_settings.hRateCostDefault` so I can wire the W2 margin-capture. (4) **fProfit* container columns** — still read by any report (PEAK)? if derive-at-read, skip rewriting. (5) **moveStatusTo99** fTransportPriceSum pre-filter load-bearing or quirk? (6) **orphan acc-payment.php** (/admin/accounting/payment · §0d orphan) — add to a menubar or retire? **+ NEXT for me: build W2→W3→W4 (in-code, no owner data) — yuan-margin part waits on flag 3.**
