...
>
> **🧾 slip-verify queue (the owner's repeated pain — finally LIVE-VERIFIED):** the dashboard ชำระเงิน tab showed (a) broken slip link (bare filename as href) → now signed-URL **thumbnail** + `SlipImage` onError fallback; (b) **the SAME customer+amount on 2 rows** = the legacy "เติม-แล้วจ่าย" PAIR (slip-bearing TOPUP type='1' + no-slip PAY type='4' reforder2→topup) → **COLLAPSED to 1 row** (drop the pay-half · tag "ชำระค่าฝากนำเข้า #F" · badge deduped). VERIFIED on prod: PR7429 ฿2,085.93 = **1 row** (was 2) · queue 9→5. Learning [[pacred-domain-knowledge]] [2026-06-21].
>
