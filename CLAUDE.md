...
>
> **🟦 REMAINING NEXT-WAVE (owner pick · no autonomous default):** **(C) accounting-hub nav consolidation** (deeply-nested menubar pages → ≤3-click hub tabs · verify the real orphan count first — the survey over-counted) · **(D) partner-API carrier adapters GOGO/JMF/TTP** (L · needs each carrier's creds/endpoints) · or **activate a dormant lever** (flip `shop_yuan_enabled` / `commission.freight_enabled` / fill Customs FX + HS + GL data — owner/accountant data-entry, code is DONE).
>
