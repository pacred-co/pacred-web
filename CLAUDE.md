...
1. **LINE_STAFF_GROUP_ID** — ✅ **RESOLVED 2026-06-01** (push-tested HTTP 200). Real staff groupId = **`C09344be50f51abbfb8ca9fddb24e10f9`** ("SA-MKT-PR Pacred", 14+bot=15 — matches owner's screenshot). The `C61f…` in prod env was the chat.line.biz OA-Manager chat-thread id (404, not pushable). Found by READING ปอน's Cloudflare Worker data (it captures every event to `Podeng_line_webhook_events`) — no deploy, no edit to his Worker (owner directive: ต่อยอด ไม่ปิดกั้นน้อง). **Owner action left = Vercel Production env: `LINE_STAFF_GROUP_ID=C09344be50f51abbfb8ca9fddb24e10f9` + `LINE_PUSH_BYPASS=false` → redeploy → P1-24 fires.** Full runbook + findings: **`docs/setup/line-staff-group-activation.md`**. (Note: ปอน's Worker writes `Podeng_*` tables; our 0131 `line_*` tables are unused — reconcile later, ปอน lane.)
