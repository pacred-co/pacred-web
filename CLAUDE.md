...
# 🧾 2026-06-12 — เดฟ: cargo-acct GAP 1 cost/declared AUTO-FILL (owner ask) + integrate ภูม cargo-acct D-G2 + mig 0175-0178 all applied prod + lockfile junction root-cause → dave-pacred + MAIN · read FIRST

> **🏁 ON MAIN.** main = dave-pacred = `<HEAD>` (owner: "เอางานน้องๆ มารวม → push dave-pacred + main · ทำเลินนิ่ง · run long") · Poom-pacred / InwPond007 absorbed (0-ahead) · Vercel deploying prod. `pnpm verify` EXIT 0 (REAL · confirmed log tail through i18n-key-audit) · **migrations 0175 + 0176 + 0177 + 0178 ALL APPLIED+VERIFIED PROD this session (dry-run→apply→probe all 6 objects OK) · NEXT FREE = 0179.** Resume: `git fetch origin && git pull origin main` (needs `.env.local` · prod Supabase `yzljakczhwrpbxflnmco` · `SUPABASE_DB_PASSWORD=Jirayus40x.` · OTP_BYPASS=false).
>
> **★ GAP 1 — cost/declared AUTO-FILL (the owner's literal ask · audit `docs/research/cargo-cost-declared-workflow-audit-2026-06-11.md`):** the per-line COST + DECLARED editor rendered EMPTY (inited only from null mig-0158 columns). Now it pre-fills from the order data above, then stays editable. **NEW `lib/forwarder/cargo-cost-autofill.ts`** (+29-assertion test) — SHOP declared = `roundUp2(cprice¥ × hratecostdefault × qty)` · IMPORT declared = `round2(fcosttotalprice × qtyShare)`. `cargo-cost-line-editor.tsx`: auto seeds the draft ONLY when stored is empty (**stored always wins**) · per-field "ออโต้" chip while value==seed · read-only summary shows effective values · amber "จากราคาขาย — แก้เป็นต้นทุนจริง" hint on the ¥ cost-unit (it seeds from SELLING price) · **persists ONLY on Save (confirm §0f)**. `shop-cost-section` + `forwarder-cost-section` read `tb_settings.hratecostdefault` once + seed each line. **ISOLATION (§0e/§0f) CLEAN** — display-only seeds, never touch cprice/ftotalprice/fcosttotalprice/status/comms. **Adversarial 3-lens review (money-isolation · math · regression) = SHIP · 0 blockers** (2 nits folded in: the ¥-from-selling warning + a documented round-drift test). commit `dafa481f`.
>
> **🔀 Integrated ภูม Poom-pacred D-G2 (6 commits · FF-clean)** — `0db474a4` D-G2 อากรขาเข้า + ราคารวม VAT editor (Excel-killer · `lib/forwarder/import-duty-vat.ts` + `<ForwarderImportDutyEditor>`) · `a9db6e55` type fix · + 3 docs/learnings. **mig 0178** (`tb_forwarder.import_duty_pct/thb`) was "applied DEV, PROD pending" per ภูม's save-point → **applied PROD this session.** ปอน InwPond007 = 0-ahead (already absorbed).
>
> **🟠 Lockfile "stale" was a MISDIAGNOSIS (learning captured):** the worktree's `pnpm-lock.yaml` was NEVER stale — it HAS `ws`/`bwip-js`/`google-auth-library`. The `tsc` "cannot find module" errors came from the worktree's `node_modules` being a **junction to the OLD main-repo install** (main repo checkout sits on a stale commit whose package.json lacks the 3 deps). Fix = drop the junction + a real `pnpm install` in the worktree → all 3 present → `tsc` clean. The owner-action-item "regen lockfile" is **VOID** — nothing to regen.
>
> **🔴 OWNER ACTION ITEMS (carryover · unchanged):** 4 staff-code review cases (PR009/PR038/PR075/PR112) · `RECEIPT_TOKEN_SECRET` in Vercel · commission 50/50 + W6 tier rates · `contact@pacred.co` mailbox · flip `commission.freight_enabled` / `tax_invoice.shop_yuan_enabled` (after VAT-base sign-off + PEAK GL — mig 0177 seeded `peak.gl_accounts`, accountant fills the codes + flips `pending`) · enable `pricing`/`warehouse`/`freight_*_doc` roles · ใบขน VAT-base sign-off · NETBAY creds · rotate dev DB password · test-customer login (the §0c authed-click-test blocker — GAP 1 cost editor is admin-gated, NOT browser-tested).
