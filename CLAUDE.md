...
>
> **🟦 Wave 5 (เกร็ดเล็ก legacy-admin) = §0b-BLOCKED on this machine** — the faithful-port items (adjust-words · staff-types · VIP-group commission filter · promo reports) need the legacy PHP source to port faithfully, and the legacy source is **NOT on this machine** (only the prod/dev DB + code). Owner: to do Wave 5 faithfully I need the legacy `pcs-admin/*.php` for those screens (or descope to "Pacred-native, not faithful"). Waves 3 (payroll · salary data) + 4 (GOGO/JMF creds) remain owner-input-blocked.
