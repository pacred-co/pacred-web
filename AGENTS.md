...
## 0k. Local-first: รัน dev ค้างไว้ · อย่า deploy บ่อย (2026-07-26 · owner)

owner: *"run local ไว้เลยเลยครับ login ให้แล้ว เอาไว้ตรวจงาน และเห็นงาน draft ก่อนได้เลยครับ
ไม่ต้องขึ้น main deploy ให้เสียเงินบ่อยครับ บันทึก setting นี้ไว้เลยนะครับ ทุกเครื่องทุกคน"*

ทุก push เข้า `main` = Vercel build = **เสียเงินจริง** (เคยบาน $188) และงานหน้าตา/flow ต้อง
ให้ owner *เห็น* ก่อนตัดสิน. เพราะฉะนั้น:

1. **เปิด dev server ค้างทั้ง session** — `preview_start { name: "pacred-1to1" }` (ห้ามรันผ่าน Bash) ·
   owner login ค้างไว้ให้ → เข้า `/admin/*` ได้เลย (เราไม่พิมพ์รหัสเอง อาศัย session ที่เปิดค้าง)
2. **ตรวจบน localhost ก่อนพูดว่าเสร็จ** — render จริง คลิกจริง อ่าน DOM ยืนยันผล
   (§0c ที่เคยติดตั้งแต่ พ.ค. = ปลดล็อกแล้ว · build เขียวไม่ใช่หลักฐาน)
3. ⚠️ **dev server รันจาก worktree ไหน = เห็นโค้ดของ worktree นั้น** — ทำงานคนละ worktree
   กับที่ server รัน จะเห็นของเก่าแล้วหลงว่า "แก้ไม่ติด" · เช็ค `lsof -p <pid> | grep cwd`
   → ไม่ตรงให้ commit แล้ว `git checkout --detach <sha>` ใน worktree ที่ server รันอยู่
4. **push:** `dave-pacred2` ได้ตามปกติ (ไม่ deploy = ไม่เสียเงิน · เป็น backup งาน) ·
   **`main` = เฉพาะตอน owner สั่ง**
