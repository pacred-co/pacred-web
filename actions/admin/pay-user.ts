  ...
  return withAdmin<ReverseForwarderPaymentResult>(
    ...
    async ({ adminId }) => {
      ...
      if (hasReforder2 || hasPaydepositLink) {
        return {
          ok: false,
          // บอกเลขชุดให้ตามงานต่อได้จริง (owner 2026-07-28: ข้อความที่ชี้ไปหน้าที่ไม่มีปุ่ม = ทางตัน)
          error:
            `ออเดอร์ #${fid} ชำระรวมสลิปเดียวกับรายการอื่น (ชุดเลขที่ #${String(funding.reforder2 ?? funding.id)}) — ` +
            `ย้อนทีละออเดอร์ไม่ได้ เพราะจะคืนเงินผิดชุด · ให้บัญชีย้อนทั้งชุด หรือแจ้งทีมเทคนิคพร้อมเลขชุดนี้`,
        };
      }
