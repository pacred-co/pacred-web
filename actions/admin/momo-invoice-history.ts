...
/**
 * เพดานแถวที่ดึงมาแสดงประวัติ.
 *
 * 🔴 owner 2026-08-08: *"บางทีเราอัพเอกสารไป แต่ไม่ได้ทำอะไร ก็ไม่ได้เก็บ log ไว้
 * รีแล้วหาไม่เจอ"* — ของถูกเก็บครบมาตลอด (prod 55 แถว) แต่เพดานเดิม **50** ตัดของเก่า
 * ทิ้งไปเงียบๆ → ใบที่อัพไว้เมื่อวานหายจากจอทั้งที่อยู่ใน DB.
 * ⚠️ เพดานนี้ตัด **ก่อน** จัดกลุ่มตามเลขใบ ⇒ ยิ่งอัพซ้ำใบเดิมบ่อย ใบอื่นยิ่งหลุดเร็ว
 * (1 ใบอัพ 3 ครั้ง = กิน 3 ช่อง). ยกเป็น 500 แถว ≈ ครอบประวัติทั้งหมดที่มี
 * แล้วให้จอเป็นคนยุบเวอร์ชันซ้ำเอง (มีอยู่แล้ว).
 */
const LIST_CAP = 500;
