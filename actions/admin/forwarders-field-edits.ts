...
export async function adminReassignForwarderOwner(
  ...
  return withAdmin(["ops", "accounting", "super", "warehouse"], async ({ adminId }) => {
    ...
    // pipeline — the WHOLE transition (money-zero guard · staging crate ·
    // TOCTOU · price + split) lives in ONE brain shared with the cron
    // self-heal: lib/admin/activate-no-code-owner.ts (owner 2026-08-02).
    if (isOwnerlessSpecial) {
      const res = await activateNoCodeOwner(admin, { fId: d.fId, newUserId, legacyAdminId });
      ...
      await logAdminAction(adminId, "tb_forwarder.assign_no_code_owner", "tb_forwarder", String(d.fId), {
        ...
        to: newUserId,
        from_status: "99",
        to_status: res.data.nextStatus,
        cabinet: res.data.cabinet,
        ship_by: res.data.shipBy,
        crate_fee: res.data.cratePrice,
        staging_stamp_failed: res.data.stagingStampFailed,
        rate_ok: res.data.rateOk,
      });

      revalidatePath(`/admin/forwarders/${d.fId}`);
      revalidatePath("/admin/forwarders");
      revalidatePath("/admin/momo-containers");
      return { ok: true, data: { noCodeActivated: true, rateOk: res.data.rateOk } };
    }
