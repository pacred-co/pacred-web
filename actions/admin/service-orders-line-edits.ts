...
// ────────────────────────────────────────────────────────────
// 4. adminSetShopTracking
//    Set/clear the China tracking number `ctrackingnumber` for every
//    tb_order row of a (hno, cnameshop) shop group — the per-shop tracking
//    entry the purchasing team uses while goods ship from each China shop.
//
//    Owner directive 2026-06-04: "แต่ละร้านมี track ของตัวเอง · ออเดอร์เดียว
//    สั่ง 3 ร้าน = 3 tracking ใต้ชื่อร้าน · ให้ pricing สั่งซื้อจีนมากรอก".
//
//    Legacy SOT: shops.php L1371 (`saveTarcking` writes
//    `tb_order.cTrackingNumber` WHERE hNo+cNameShop). Here the tracking
//    RECORD is DECOUPLED from the forwarder spawn — the team records each
//    shop's tracking as it arrives, then the status-4 SpawnForwarderForm
//    (which reads ctrackingnumber via buildSpawnRows) creates the
//    tb_forwarder rows in one pass. Faithful workflow (§0a), Pacred UI.
//
//    Initial SET / clear only. To RENAME an existing tracking + cascade
//    the rename into tb_forwarder.ftrackingchn, use
//    adminUpdateCartItemCTracking (writer #2 above).
// ────────────────────────────────────────────────────────────

const setShopTrackingSchema = z.object({
  h_no:               z.string().regex(/^P\d+$/, "hno ต้องเป็นรูปแบบ P{digits}").max(30),
  c_name_shop:        z.string().trim().min(1, "ระบุชื่อร้านจีน").max(300),
  c_tracking_number:  z.string().trim().max(500),     // empty allowed → clear
});
export type AdminSetShopTrackingInput = z.infer<typeof setShopTrackingSchema>;

type SetShopTrackingData = {
  hno:                string;
  c_name_shop:        string;
  c_tracking_number:  string;
  rows_touched:       number;
};

export async function adminSetShopTracking(
  input: AdminSetShopTrackingInput,
): Promise<AdminActionResult<SetShopTrackingData>> {
  const parsed = setShopTrackingSchema.safeParse(input);
  if (!parsed.success) {
    return { ok: false, error: parsed.error.issues[0]?.message ?? "invalid_input" };
  }
  const d = parsed.data;

  return withAdmin<SetShopTrackingData>(
    ["super", "accounting", "ops", "sales_admin"],
    async ({ adminId }) => {
      const admin = createAdminClient();
      const legacyAdminId = safeLegacyAdminId(await resolveLegacyAdminId(), 10);

      // 1. Pre-read the shop's lines (audit before/after + refund guard).
      const { data: before, error: beforeErr } = await admin
        .from("tb_order")
        .select("id, hno, cnameshop, ctrackingnumber, crewallet")
        .eq("hno", d.h_no)
        .eq("cnameshop", d.c_name_shop) as unknown as {
          data: Array<{
            id: number; hno: string; cnameshop: string;
            ctrackingnumber: string | null; crewallet: string | null;
          }> | null;
          error: { code?: string; message?: string } | null;
        };
      if (beforeErr) {
        console.error(`[tb_order shop-tracking pre-read] failed`, {
          code: beforeErr.code, message: beforeErr.message,
        });
        return { ok: false, error: `db_error:${beforeErr.code ?? "unknown"}` };
      }
      if (!before || before.length === 0) {
        return { ok: false, error: `ไม่พบรายการของร้าน "${d.c_name_shop}" ในออเดอร์ ${d.h_no}` };
      }

      // Skip already-refunded lines (legacy update.php L98 disabled state).
      const eligible = before.filter((r) => r.crewallet !== "1");
      if (eligible.length === 0) {
        return {
          ok: false,
          error: `รายการของร้าน "${d.c_name_shop}" ถูกคืนเงินทั้งหมดแล้ว — บันทึก tracking ไม่ได้`,
        };
      }

      // 2. Idempotency — no-op if every eligible row already has this value.
      const allMatch = eligible.every(
        (r) => (r.ctrackingnumber ?? "") === d.c_tracking_number,
      );
      if (allMatch) {
        return {
          ok: true,
          data: {
            hno: d.h_no, c_name_shop: d.c_name_shop,
            c_tracking_number: d.c_tracking_number, rows_touched: 0,
          },
        };
      }

      // 3. UPDATE — scoped on (hno, cnameshop), excluding refunded lines.
      const { error: updErr, count } = await admin
        .from("tb_order")
        .update({ ctrackingnumber: d.c_tracking_number }, { count: "exact" })
        .eq("hno", d.h_no)
        .eq("cnameshop", d.c_name_shop)
        .neq("crewallet", "1") as unknown as {
          error: { code?: string; message?: string } | null;
      ...
      if (updErr) {
        console.error(`[tb_order ctrackingnumber set] failed`, {
          code: updErr.code, message: updErr.message,
        });
        return { ok: false, error: `บันทึกเลข tracking ร้านจีนล้มเหลว: ${updErr.message}` };
      }
      const rowsTouched = typeof count === "number" ? count : eligible.length;

      // 4. Stamp parent header audit (non-fatal).
      const nowIso = new Date().toISOString();
      const { error: hdrStampErr } = await admin
        .from("tb_header_order")
        .update({ hdateupdate: nowIso, adminidupdate: legacyAdminId })
        .eq("hno", d.h_no);
      if (hdrStampErr) {
        console.error(`[tb_header_order audit-stamp (tracking)] failed (non-fatal)`, {
          code: hdrStampErr.code, message: hdrStampErr.message,
        });
      }
      ...
      await logAdminAction(
        adminId,
        "tb_order.set_ctrackingnumber",
        "tb_order",
        d.h_no,
        {
          hno: d.h_no, c_name_shop: d.c_name_shop,
          before_values: before.map((r) => ({
            id: r.id, ctrackingnumber: r.ctrackingnumber ?? "", crewallet: r.crewallet ?? "",
          })),
          after_value: d.c_tracking_number, rows_touched: rowsTouched,
        },
      );
      ...
      return {
        ...
        data: {
          hno: d.h_no, c_name_shop: d.c_name_shop,
          c_tracking_number: d.c_tracking_number, rows_touched: rowsTouched,
        },
      };
    },
  );
