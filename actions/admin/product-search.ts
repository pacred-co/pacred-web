...
export async function searchProductByUrlAdmin(
  ...
  return withAdmin<AdminProductSearchOk>([...SEARCH_ROLES], async () => {
    ...
    if (isDemo) {
      // owner 2026-08-04: แยก "ไม่มีสินค้านี้จริง" ออกจาก "TAMIT ตอบไม่ทัน" — พนักงาน
      // เจอปัญหาเดียวกับลูกค้า (ขึ้นว่าไม่พบ ทั้งที่ลองใหม่แล้วได้). ฝั่งเซิร์ฟเวอร์
      // ลองซ้ำให้แล้ว (lib/china-search/tamit-detail-fetch.ts) — ข้อความนี้คือกรณี
      // ที่ลองครบแล้วยังไม่ได้ จึงต้องบอกตรงๆ ว่าให้ลองใหม่ ไม่ใช่บอกว่าไม่มีของ.
      return {
        ok: false,
        error: d.fallback_reason === "unreachable"
          ? "ระบบดึงข้อมูลสินค้าช้ากว่าปกติ กรุณาลองใหม่อีกครั้ง หรือกรอกรายการสินค้าเอง"
